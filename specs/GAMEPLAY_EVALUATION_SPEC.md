...
**Status:** Normative evaluation catalog and interface; executable evaluator registries not yet materialized

**Date:** 2026-08-12

**Scope:** Gameplay behavior, simulation regressions, and presentation-dependent gameplay feel

**Catalog version:** `gameplay-evaluation-v2`

## 1. Purpose and authority

This specification transforms the PES 2017 Reference Test Catalog and its measurement method into a normative catalog and interface that a scenario runner, metric library, reference registry, and promotion evaluator can implement. It is not an executable evaluator package and does not claim that the engine currently matches PES 2017.
...
- catalog records define required evaluation intent, but their prose scenarios, observables, metrics, and rules are not executable definitions;
- no catalog test may return `PASS` or `FAIL` until every required registry object, schema, binding, and protected oracle resolves and validates;
...
The catalog in §7 is normative structured YAML, but its `initial_scenario`, `controlled_inputs`, `state_to_record`, `metrics`, and criterion `rule` fields are descriptive requirements, not executable programs. Each family block is a standalone document fragment with a `tests` array; a materializer concatenates those arrays in document order and rejects duplicate IDs. Executability begins only when versioned machine-readable registries under `eval/contracts/` implement the interfaces below and a binding resolves a catalog record to those objects. The suite and tier blocks in §8 are registry documents. Field names and enum values MUST remain stable within a catalog version.
...
`test_id` is the stable catalog key, not necessarily a one-to-one scenario filename. The catalog's `metrics` are required logical observables. Before a test implementation can report `PASS` or `FAIL`, a versioned `TestImplementationBinding` MUST resolve every applicable criterion and descriptive requirement to executable scenario, observation, metric, invariant, schema, and policy registry IDs. Registry loading rejects duplicate IDs, unresolved references, version mismatches, unrecognized fields, schema-invalid scenario data, and bindings that omit a required logical observable. A missing or invalid dependency yields `INVALID_RUN` when the test is required by the selected milestone and `NOT_EVALUATED` only when it is optional diagnostic. A `ReferenceTarget` is valid only after the evidence gates in §5.1 pass, and its `acceptance_parameters` remain empty until evidence supports actual tolerances.

The evaluator package MUST publish a registry-set ID and content hash covering every loaded definition and schema. A result records the exact resolved IDs/versions; prose text is never executed as a fallback. Until these registries and bindings exist, this document remains a catalog/interface and no catalog entry is promotion-executable.
...
These criteria apply to every catalog test unless the scenario declares a documented non-applicability reason. A family record's `regression_impact_ids` expands targeted regression scope; it does not replace these criteria.
...
The `ReferenceTarget` registry is the sole authority for whether a target exists, is eligible, and is current. At evaluation time, the evaluator joins it to the catalog by `(test_id, criterion_id)`, verifies that the registry `reference_class` matches the catalog's expected `reference_evidence.measurement_class`, and records the resolved target/version/stratum. No match means `BLOCKED_MISSING_REFERENCE` for an applicable measured criterion; a class mismatch, duplicate active target, or schema/version error is `INVALID_RUN`. Catalog text never caches mutable target presence.
...
These rules constrain the executable registries; they are not substitute estimator implementations. Each metric registry entry MUST still declare its exact inputs, units, estimator/version, filters, observation-window and boundary behavior, invalid-data behavior, and output schema/version.
...
Catalog reference metadata states only the expected measurement class and evidence limitation. Target presence, eligibility, current class, strata, and versions are owned solely by the `ReferenceTarget` registry described in §5. Behavioral evidence labels point to the matching row and surrounding analysis in [01-pes2027-behavior.md](../research/01-pes2027-behavior.md); measurement limitations come from [02-reference-measurement.md](../research/02-reference-measurement.md). Claim-level external citation mapping remains unresolved under Audit F-04, so the evidence field does not pretend otherwise.
...
tests:
  - test_id: LOC-ACC-001
    ...
    reference_evidence: {behavioral: "R1 LOC-ACC-001; gradual rapid progression observed", measurement_class: B, evidence_limit: "Exact PES command frame and player metadata are hidden"}
    ...
    regression_impact_ids: [LOC-DEC-001, LOC-T45-001, LOC-T90-001, CTRL-LAT-001]
  ...
  - test_id: LOC-ACC-002
    ...
    reference_evidence: {behavioral: "R1 LOC-ACC-002; separate Speed and Explosive Power dimensions", measurement_class: C, evidence_limit: "Public players differ in many attributes; causal isolation absent"}
    ...
    regression_impact_ids: [LOC-ACC-001, LOC-MAX-001]
  ...
  - test_id: LOC-MAX-001
    ...
    reference_evidence: {behavioral: "R1 LOC-MAX-001; plateau expected in long sprint", measurement_class: A, evidence_limit: "Reference scale, stamina, and context must be qualified"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [LOC-ACC-001, LOC-BALL-001]
  ...
  - test_id: LOC-DEC-001
    ...
    reference_evidence: {behavioral: "R1 LOC-DEC-001; residual displacement and weight observed", measurement_class: B, evidence_limit: "Exact PES stick-release tick hidden"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [LOC-ACC-001, LOC-REV-001, CTRL-LAT-001]
  ...
  - test_id: LOC-REV-001
    ...
    reference_evidence: {behavioral: "R1 LOC-REV-001; brake-pivot-reaccelerate sequence observed", measurement_class: B, evidence_limit: "Exact PES command timing/magnitude hidden; radius unstable near zero speed"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [LOC-DEC-001, LOC-T45-001, LOC-T90-001, LOC-ORI-001]
  ...
  - test_id: LOC-T45-001
    ...
    reference_evidence: {behavioral: "R1 LOC-T45-001; moderate speed loss", measurement_class: B, evidence_limit: "Reference stick angle unknown; observed deflection only"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [LOC-T90-001, LOC-REV-001, LOC-ORI-001]
  ...
  - test_id: LOC-T90-001
    ...
    reference_evidence: {behavioral: "R1 LOC-T90-001; sharper reorientation than 45-degree turn", measurement_class: B, evidence_limit: "Reference stick angle hidden"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [LOC-T45-001, LOC-REV-001, LOC-ORI-001, CTRL-LAT-001]
  ...
  - test_id: LOC-ORI-001
    ...
    reference_evidence: {behavioral: "R1 LOC-ORI-001; torso and velocity need not coincide", measurement_class: B, evidence_limit: "Monocular pose estimation is uncertain"}
    ...
    regression_impact_ids: [LOC-T45-001, LOC-T90-001, LOC-REV-001, CTRL-LAT-001]
  ...
  - test_id: LOC-BALL-001
    ...
    reference_evidence: {behavioral: "R1 LOC-BALL-001; periodic touches and possible speed cost", measurement_class: C, evidence_limit: "No matched public PES trial"}
    ...
    regression_impact_ids: [BALL-IND-001, LOC-MAX-001, TOUCH-SLOW-001]
```
...
tests:
  - test_id: PHY-SHLD-001
    ...
    reference_evidence: {behavioral: "R1 PHY-SHLD-001; continuous shoulder-to-shoulder outcome", measurement_class: B, evidence_limit: "Reference contact input/state hidden"}
    ...
    regression_impact_ids: [PHY-STR-001, PHY-BC-001, BALL-IND-001]
  ...
  - test_id: PHY-STR-001
    ...
    reference_evidence: {behavioral: "R1 PHY-STR-001; roster supports strength/agility distinction", measurement_class: C, evidence_limit: "No isolated matched PES pair"}
    target_types: [ENGINE_DESIGN_TARGET, UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PHY-SHLD-001, PHY-BC-001]
  ...
  - test_id: PHY-BC-001
    ...
    reference_evidence: {behavioral: "R1 PHY-BC-001; some players maintain/recover posture better", measurement_class: C, evidence_limit: "Attribute-specific causal trial absent"}
    target_types: [ENGINE_DESIGN_TARGET, UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PHY-SHLD-001, PHY-STR-001, LOC-ORI-001]
  ...
  - test_id: PHY-PC-001
    ...
    reference_evidence: {behavioral: "R1 PHY-PC-001; low-confidence community roster intervention", measurement_class: C, evidence_limit: "Community mod uncontrolled and indirect effects possible"}
    target_types: [ENGINE_DESIGN_TARGET, UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PHY-SHLD-001, TACK-ANG-001, TEMPO-001]
  ...
  - test_id: BALL-IND-001
    ...
    reference_evidence: {behavioral: "R1 BALL-IND-001; loose balls/rebounds behave independently", measurement_class: A, evidence_limit: "Invisible PES capture assistance remains possible"}
    ...
    regression_impact_ids: [LOC-BALL-001, TOUCH-SLOW-001, INT-PASS-001, GK-PARRY-001]
  ...
  - test_id: BALL-GND-001
    ...
    reference_evidence: {behavioral: "R1 BALL-GND-001; roll-off visible", measurement_class: A, evidence_limit: "Initial input power unknown but free decay measurable"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-GND-002, PASS-LOW-001, TOUCH-SLOW-001]
  ...
  - test_id: BALL-GND-002
    ...
    reference_evidence: {behavioral: "R1 BALL-GND-002; compare passes of different speeds", measurement_class: B, evidence_limit: "Reference initial spin/contact/surface may confound"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-GND-001, PASS-LOW-001]
  ...
  - test_id: BALL-BNC-001
    ...
    reference_evidence: {behavioral: "R1 BALL-BNC-001; visible first bounce", measurement_class: B, evidence_limit: "Public monocular vertical/spin reconstruction is weak"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-GND-001, BALL-SPN-001]
  ...
  - test_id: BALL-SPN-001
    ...
    reference_evidence: {behavioral: "R1 BALL-SPN-001; Swerve dimension and visible curve", measurement_class: B, evidence_limit: "3D, camera, and spin separation difficult"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-SPN-002, BALL-BNC-001, SHOT-SWV-001]
  ...
  - test_id: BALL-SPN-002
    ...
    reference_evidence: {behavioral: "R1 BALL-SPN-002; proposed curve-versus-power comparison", measurement_class: C, evidence_limit: "Known repeated PES power inputs unavailable"}
    ...
    regression_impact_ids: [BALL-SPN-001, SHOT-PWR-001]
```
...
tests:
  - test_id: TOUCH-SLOW-001
    ...
    reference_evidence: {behavioral: "R1 TOUCH-SLOW-001; short control and early availability", measurement_class: B, evidence_limit: "Receiver intent hidden in public footage"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TOUCH-FAST-001, BALL-IND-001, PASS-LOW-001]
  ...
  - test_id: TOUCH-FAST-001
    ...
    reference_evidence: {behavioral: "R1 TOUCH-FAST-001; speed/height affect Real Touch", measurement_class: B, evidence_limit: "Matched public intent unavailable"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TOUCH-SLOW-001, BALL-GND-001, PASS-LOW-001]
  ...
  - test_id: TOUCH-BACK-001
    ...
    reference_evidence: {behavioral: "R1 TOUCH-BACK-001; orientation and manner of receiving matter", measurement_class: B, evidence_limit: "Exact input and pose are uncertain"}
    target_types: [MEASURED_TARGET, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TOUCH-90-001, TOUCH-SLOW-001, LOC-ORI-001]
  ...
  - test_id: TOUCH-90-001
    ...
    reference_evidence: {behavioral: "R1 TOUCH-90-001; side-on control differs", measurement_class: B, evidence_limit: "Reference pose and intent uncertain"}
    target_types: [MEASURED_TARGET, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TOUCH-BACK-001, TOUCH-SLOW-001, LOC-ORI-001]
  ...
  - test_id: TOUCH-WF-001
    ...
    reference_evidence: {behavioral: "R1 TOUCH-WF-001; weak-foot fields exist but Real Touch link unconfirmed", measurement_class: C, evidence_limit: "Specific causal relation unknown"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TOUCH-90-001, PASS-ANG-001]
  ...
  - test_id: PASS-LOW-001
    ...
    reference_evidence: {behavioral: "R1 PASS-LOW-001; attributes, orientation, timing, weight/speed matter", measurement_class: B, evidence_limit: "Reference aim, power bar, assistance unknown"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-IND-001, BALL-GND-001, TOUCH-SLOW-001, CTRL-ACT-001]
  ...
  - test_id: PASS-ANG-001
    ...
    reference_evidence: {behavioral: "R1 PASS-ANG-001; official evidence says direction/angle matter", measurement_class: C, evidence_limit: "Causal penalty needs controlled matched attempts"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PASS-LOW-001, LOC-ORI-001, CTRL-ACT-001]
  ...
  - test_id: PASS-RUN-001
    ...
    reference_evidence: {behavioral: "R1 PASS-RUN-001; teammate motion is considered", measurement_class: B, evidence_limit: "Public intended target and assistance unknown"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PASS-LOW-001, INT-PASS-001, OFF-RUN-001]
  ...
  - test_id: PASS-THR-001
    ...
    reference_evidence: {behavioral: "R1 PASS-THR-001; reviewer observed some underpowered short through passes", measurement_class: B, evidence_limit: "Subjective, unquantified sample and hidden command"}
    target_types: [MEASURED_TARGET, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PASS-LOW-001, PASS-RUN-001, INT-PASS-001]
  ...
  - test_id: PASS-LOFT-001
    ...
    reference_evidence: {behavioral: "R1 PASS-LOFT-001; contextual lofted variety", measurement_class: B, evidence_limit: "Public 3D reconstruction and power input uncertain"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-BNC-001, BALL-SPN-001, PASS-LOW-001]
  ...
  - test_id: CROSS-HI-001
    ...
    reference_evidence: {behavioral: "R1 CROSS-HI-001; high curved variants documented", measurement_class: B, evidence_limit: "3D and assistance/input uncertain"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PASS-LOFT-001, HEAD-FREE-001, GK-HIGH-001]
  ...
  - test_id: SHOT-PWR-001
    ...
    reference_evidence: {behavioral: "R1 SHOT-PWR-001; power bar and Kicking Power dimensions exist", measurement_class: C, evidence_limit: "Public footage cannot recover power-bar ladder"}
    target_types: [UNKNOWN, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [SHOT-IND-001, BALL-SPN-002, CTRL-ACT-001]
  ...
  - test_id: SHOT-IND-001
    ...
    reference_evidence: {behavioral: "R1 SHOT-IND-001; players with differing Kicking Power are observable", measurement_class: C, evidence_limit: "Public player comparison is confounded"}
    target_types: [ENGINE_DESIGN_TARGET, UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [SHOT-PWR-001, SHOT-SWV-001]
  ...
  - test_id: SHOT-SWV-001
    ...
    reference_evidence: {behavioral: "R1 SHOT-SWV-001; Swerve is a separate roster dimension", measurement_class: C, evidence_limit: "Controlled comparable strikes absent"}
    target_types: [ENGINE_DESIGN_TARGET, UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-SPN-001, BALL-SPN-002, SHOT-PWR-001]
  ...
  - test_id: HEAD-FREE-001
    ...
    reference_evidence: {behavioral: "R1 HEAD-FREE-001; Header and Jump dimensions; positioning/timing matter", measurement_class: B, evidence_limit: "Input timing and public 3D height uncertain"}
    ...
    regression_impact_ids: [CROSS-HI-001, HEAD-DUEL-001, BALL-BNC-001]
  ...
  - test_id: HEAD-DUEL-001
    ...
    reference_evidence: {behavioral: "R1 HEAD-DUEL-001; jump, position, and physical interaction", measurement_class: B, evidence_limit: "3D and contact-selection ambiguity"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [HEAD-FREE-001, PHY-SHLD-001, CROSS-HI-001]
```
...
tests:
  - test_id: TACK-ST-001
    ...
    reference_evidence: {behavioral: "R1 TACK-ST-001; lunge/contact/recovery action documented", measurement_class: B, evidence_limit: "PES command time and internal hitbox/window hidden"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PHY-SHLD-001, TACK-ANG-001, BALL-IND-001]
  ...
  - test_id: TACK-SL-001
    ...
    reference_evidence: {behavioral: "R1 TACK-SL-001; timing/risk and commitment documented", measurement_class: B, evidence_limit: "Command and collision internals hidden"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TACK-ST-001, TACK-ANG-001, PHY-SHLD-001]
  ...
  - test_id: TACK-ANG-001
    ...
    reference_evidence: {behavioral: "R1 TACK-ANG-001; proposed 0/30/60/90-degree experiment", measurement_class: C, evidence_limit: "Controlled PES angle curve absent"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TACK-ST-001, TACK-SL-001, PHY-SHLD-001]
  ...
  - test_id: INT-PASS-001
    ...
    reference_evidence: {behavioral: "R1 INT-PASS-001; proactive but imperfect interception", measurement_class: B, evidence_limit: "Decision trigger and awareness hidden"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PASS-LOW-001, INT-FAST-001, DEF-SHAPE-001]
  ...
  - test_id: INT-FAST-001
    ...
    reference_evidence: {behavioral: "R1 INT-FAST-001; fast passes reduce opportunity, with observed imperfections", measurement_class: C, evidence_limit: "Matched controlled PES speeds absent"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [INT-PASS-001, PASS-LOW-001, LOC-ACC-001]
  ...
  - test_id: GK-REA-001
    ...
    reference_evidence: {behavioral: "R1 GK-REA-001; immediate/reworked keeper reactions", measurement_class: B, evidence_limit: "Visible latency may include anticipation before shot contact"}
    target_types: [MEASURED_TARGET, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [GK-WF-001, GK-LEG-001, GK-REC-001]
  ...
  - test_id: GK-WF-001
    ...
    reference_evidence: {behavioral: "R1 GK-WF-001; wrong-foot reactions explicitly described", measurement_class: C, evidence_limit: "Repeatable known pre-motion/shot states require controlled capture"}
    ...
    regression_impact_ids: [GK-REA-001, LOC-REV-001]
  ...
  - test_id: GK-LEG-001
    ...
    reference_evidence: {behavioral: "R1 GK-LEG-001; foot saves documented/observed", measurement_class: B, evidence_limit: "3D rebound and selected animation uncertain"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [GK-REA-001, GK-PARRY-001, GK-REC-001]
  ...
  - test_id: GK-PARRY-001
    ...
    reference_evidence: {behavioral: "R1 GK-PARRY-001; official evidence says body part affects deflection", measurement_class: B, evidence_limit: "Public 3D, occlusion, and nonphysical correction uncertain"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [BALL-IND-001, GK-LEG-001, GK-REC-001]
  ...
  - test_id: GK-REC-001
    ...
    reference_evidence: {behavioral: "R1 GK-REC-001; chained action after falling explicitly described", measurement_class: B, evidence_limit: "Natural initial states vary"}
    target_types: [MEASURED_TARGET, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [GK-REA-001, GK-LEG-001, GK-PARRY-001]
  ...
  - test_id: GK-HIGH-001
    ...
    reference_evidence: {behavioral: "R1 GK-HIGH-001; stronger claims/catches observed", measurement_class: B, evidence_limit: "Aerial geometry and decision threshold uncertain"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [CROSS-HI-001, HEAD-DUEL-001, GK-PARRY-001]
```
...
tests:
  - test_id: OFF-RUN-001
    ...
    reference_evidence: {behavioral: "R1 OFF-RUN-001; Player ID and observed legible forward runs", measurement_class: B, evidence_limit: "Autonomous versus user-triggered cause may be unknown"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [OFF-SUP-001, DEF-SHAPE-001, PASS-RUN-001]
  ...
  - test_id: OFF-SUP-001
    ...
    reference_evidence: {behavioral: "R1 OFF-SUP-001; Team ID/gameplay support movement", measurement_class: B, evidence_limit: "Tactical context and intent incomplete"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [OFF-RUN-001, DEF-SHAPE-001, TACT-SUP-001]
  ...
  - test_id: DEF-SHAPE-001
    ...
    reference_evidence: {behavioral: "R1 DEF-SHAPE-001; centre-backs observed preserving position", measurement_class: B, evidence_limit: "Full team may be cropped and tactics unknown"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [DEF-SHIFT-001, PRESS-001, INT-PASS-001, OFF-SUP-001]
  ...
  - test_id: DEF-SHIFT-001
    ...
    reference_evidence: {behavioral: "R1 DEF-SHIFT-001; block recenters/deforms", measurement_class: B, evidence_limit: "Formation/tactics and full visibility uncertain"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [DEF-SHAPE-001, PRESS-001, TACT-COMP-001]
  ...
  - test_id: PRESS-001
    ...
    reference_evidence: {behavioral: "R1 PRESS-001; frontline pressure option exists", measurement_class: B, evidence_limit: "Natural episodes do not identify slider mapping"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [DEF-SHAPE-001, DEF-SHIFT-001, TRANS-AD-001]
  ...
  - test_id: PRESS-GG-001
    ...
    reference_evidence: {behavioral: "R1 PRESS-GG-001; Gegenpress instruction exists", measurement_class: C, evidence_limit: "Known instruction state and repeated losses absent in public data"}
    target_types: [UNKNOWN, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PRESS-REC-001, PRESS-001, TRANS-AD-001]
  ...
  - test_id: PRESS-REC-001
    ...
    reference_evidence: {behavioral: "R1 PRESS-REC-001; proposed recovery behavior", measurement_class: C, evidence_limit: "PES abandonment rule not documented and instruction state required"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PRESS-GG-001, DEF-SHAPE-001, TRANS-AD-001]
  ...
  - test_id: TACT-COMP-001
    ...
    reference_evidence: {behavioral: "R1 TACT-COMP-001; compactness slider exists", measurement_class: C, evidence_limit: "Known settings/repeated PES states required"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [DEF-SHAPE-001, DEF-SHIFT-001, TACT-SUP-001]
  ...
  - test_id: TACT-DLINE-001
    ...
    reference_evidence: {behavioral: "R1 TACT-DLINE-001; Defensive Line setting exists", measurement_class: C, evidence_limit: "Known repeated settings required; mentality interaction"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [DEF-SHAPE-001, OFF-RUN-001, TRANS-AD-001]
  ...
  - test_id: TACT-SUP-001
    ...
    reference_evidence: {behavioral: "R1 TACT-SUP-001; Support Range setting exists", measurement_class: C, evidence_limit: "Controlled known settings absent"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [OFF-SUP-001, TACT-COMP-001, PASS-RUN-001]
  ...
  - test_id: TACT-TIKI-001
    ...
    reference_evidence: {behavioral: "R1 TACT-TIKI-001; Tiki-taka instruction exists", measurement_class: C, evidence_limit: "On/off PES comparison absent"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [OFF-SUP-001, TACT-SUP-001, TEMPO-001]
  ...
  - test_id: TACT-MARK-001
    ...
    reference_evidence: {behavioral: "R1 TACT-MARK-001; Tight Marking exists", measurement_class: C, evidence_limit: "Known assignment state and logic require controlled capture"}
    target_types: [UNKNOWN, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [DEF-SHAPE-001, INT-PASS-001]
  ...
  - test_id: AI-ADAPT-001
    ...
    reference_evidence: {behavioral: "R1 AI-ADAPT-001; Adaptive AI double-marking behavior announced", measurement_class: C, evidence_limit: "Repetition threshold/memory needs controlled sequences"}
    target_types: [UNKNOWN, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TACT-MARK-001, DEF-SHAPE-001, AI-ADAPT-002]
  ...
  - test_id: AI-ADAPT-002
    ...
    reference_evidence: {behavioral: "R1 AI-ADAPT-002; flank adaptation announced", measurement_class: C, evidence_limit: "Controlled repetitions required"}
    target_types: [UNKNOWN, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [DEF-SHIFT-001, AI-ADAPT-001]
  ...
  - test_id: TRANS-AD-001
    ...
    reference_evidence: {behavioral: "R1 TRANS-AD-001; total team control and post-loss transitions", measurement_class: B, evidence_limit: "Tactical cause/settings uncertain"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PRESS-001, PRESS-GG-001, PRESS-REC-001, DEF-SHAPE-001]
  ...
  - test_id: TRANS-DA-001
    ...
    reference_evidence: {behavioral: "R1 TRANS-DA-001; Team ID/gameplay transition behavior", measurement_class: B, evidence_limit: "Tactic settings/context uncertain"}
    target_types: [MEASURED_TARGET, HARD_INVARIANT, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [OFF-RUN-001, OFF-SUP-001, DEF-SHAPE-001]
```
...
tests:
  - test_id: CTRL-LAT-001
    ...
    reference_evidence: {behavioral: "R1 CTRL-LAT-001; immediate intention coexists with weight", measurement_class: C, evidence_limit: "True PES input timestamp absent"}
    ...
    regression_impact_ids: [LOC-ACC-001, LOC-DEC-001, LOC-T90-001, LOC-ORI-001]
  ...
  - test_id: CTRL-ACT-001
    ...
    reference_evidence: {behavioral: "R1 CTRL-ACT-001; action responsiveness reported", measurement_class: C, evidence_limit: "PES command timestamp absent"}
    target_types: [UNKNOWN, HARD_INVARIANT, PERCEPTUAL_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [PASS-LOW-001, SHOT-PWR-001, LOC-DEC-001]
  ...
  - test_id: CAM-FLW-001
    ...
    reference_evidence: {behavioral: "R1 CAM-FLW-001; raw gameplay camera motion", measurement_class: A, evidence_limit: "PES preset/build must be known or stratified"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [CAM-PER-001, LOC-MAX-001, TEMPO-003]
  ...
  - test_id: CAM-PER-001
    ...
    reference_evidence: {behavioral: "R1 CAM-PER-001; camera affects perceived speed", measurement_class: D, evidence_limit: "Perception has no unique physical scalar; equivalent captures difficult"}
    ...
    regression_impact_ids: [CAM-FLW-001, LOC-MAX-001, TEMPO-003]
  ...
  - test_id: TEMPO-001
    ...
    reference_evidence: {behavioral: "R1 TEMPO-001; complete match sequences", measurement_class: A, evidence_limit: "Event definitions and match settings must be fixed; current full-match rules incomplete"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TEMPO-002, TRANS-AD-001, TRANS-DA-001, PASS-LOW-001, TACK-ST-001]
  ...
  - test_id: TEMPO-002
    ...
    reference_evidence: {behavioral: "R1 TEMPO-002; transitions relatively quick while retaining inertia", measurement_class: A, evidence_limit: "Team tactics and event definitions condition results"}
    target_types: [MEASURED_TARGET, REGRESSION]
    acceptance_logic:
    ...
    regression_impact_ids: [TRANS-AD-001, TRANS-DA-001, TEMPO-001]
  ...
  - test_id: TEMPO-003
    ...
    reference_evidence: {behavioral: "R1 TEMPO-003; 'fast but with weight' aggregate goal", measurement_class: D, evidence_limit: "No unique scalar and PES input timing unavailable"}
    ...
    regression_impact_ids: [CTRL-LAT-001, LOC-DEC-001, LOC-T90-001, TOUCH-FAST-001, TEMPO-002, CAM-FLW-001]
```

## 8. Suite topology

The evaluator MUST expose uniform `SuiteDefinition` and `TierDefinition` registries. `regression_impact_ids` are symmetric-or-directional change-impact associations, not execution prerequisites. When closure is enabled, start with the direct test set, traverse impact IDs using a visited set and a lexically sorted frontier, and stop at the fixed point. Cycles are valid and cannot cause repeated execution. Common criteria are attached after expansion and are not graph vertices.
...
suites:
  ...
  - suite_id: locomotion
    ...
    direct_test_ids: [LOC-ACC-001, LOC-ACC-002, LOC-MAX-001, LOC-DEC-001,
  ...
  - suite_id: ball
    ...
    direct_test_ids: [BALL-IND-001, BALL-GND-001, BALL-GND-002, BALL-BNC-001,
  ...
  - suite_id: touch_and_actions
    ...
    direct_test_ids: [TOUCH-SLOW-001, TOUCH-FAST-001, TOUCH-BACK-001,
  ...
  - suite_id: duels
    ...
    direct_test_ids: [PHY-SHLD-001, PHY-STR-001, PHY-BC-001, PHY-PC-001,
  ...
  - suite_id: goalkeepers
    ...
    held_out_policy_id: null
    browser_case_ids: [BROWSER-GOALKEEPER-CONTACT-001]
    resource_policy_id: resources-browser-family-v1
    outcome_reduction_profile_id: profile-selected-v1
    expected_expansion_manifest_id: expansion-goalkeepers-v1
  - suite_id: team
    suite_version: suite-team-v1
    direct_test_ids: [OFF-RUN-001, OFF-SUP-001, DEF-SHAPE-001, DEF-SHIFT-001,
  ...
  - suite_id: browser_perceptual
    suite_version: suite-browser-perceptual-v1
    direct_test_ids: [LOC-ORI-001, TOUCH-BACK-001, TOUCH-90-001,
    ...
    common_criterion_ids: [COMMON-FINITE, COMMON-DETERMINISTIC, COMMON-REFERENCES]
    impact_closure: NONE
    prerequisite_capabilities: [PRESENTATION_BASELINE]
    seed_matrix_id: seeds-browser-v1
    config_matrix_id: config-browser-v1
    held_out_policy_id: null
    browser_case_ids: [BROWSER-EVENT-FRAMES-001]
    resource_policy_id: resources-browser-family-v1
    outcome_reduction_profile_id: perceptual-policy-selected-v1
    expected_expansion_manifest_id: expansion-browser-perceptual-v1
  - suite_id: match_ecology
    suite_version: suite-match-ecology-v1
    direct_test_ids: [TEMPO-001, TEMPO-002, TEMPO-003, TRANS-AD-001,
      TRANS-DA-001, DEF-SHAPE-001, PRESS-001]
    common_criterion_ids: [COMMON-FINITE, COMMON-DETERMINISTIC, COMMON-REFERENCES, COMMON-BOUNDS]
    impact_closure: NONE
    prerequisite_capabilities: [RULES_SPEC_V1, REGULATION_RULES_SUITE_PASS,
      GOALKEEPER_RULES_V1, GOALKEEPER_SUITE_PASS, BALL_IN_PLAY_CLOCK_V1,
      SAME_TICK_RULE_ARBITRATION_V1]
    seed_matrix_id: seeds-match-v1
    config_matrix_id: config-regulation-match-v1
    held_out_policy_id: held-out-match-v1
    browser_case_ids: [BROWSER-MATCH-SMOKE-001]
    resource_policy_id: resources-match-v1
    outcome_reduction_profile_id: regulation-promotion-v1
    expected_expansion_manifest_id: expansion-match-ecology-v1
```

```yaml
tiers:
  - {tier_id: FAST, tier_version: tier-fast-v1, suite_selection: FIXED,
     suite_ids: [fast], impact_closure: NONE, seed_matrix_id: seeds-smoke-v1,
     config_matrix_id: config-default-v1, held_out_policy_id: null,
     browser_case_policy_id: browser-none-v1, resource_policy_id: resources-fast-v1,
     outcome_reduction_profile_id: required-hard-v1}
  - {tier_id: TARGETED, tier_version: tier-targeted-v1, suite_selection: CHANGED_FAMILY,
     suite_ids: [], impact_closure: NONE, seed_matrix_id: seeds-family-v1,
     config_matrix_id: config-changed-family-v1, held_out_policy_id: null,
     browser_case_policy_id: browser-declared-paths-v1, resource_policy_id: resources-targeted-v1,
     outcome_reduction_profile_id: profile-selected-v1}
  - {tier_id: DEEP, tier_version: tier-deep-v1, suite_selection: CHANGED_FAMILY_WITH_IMPACT,
     suite_ids: [], impact_closure: REACHABLE_FIXED_POINT, seed_matrix_id: seeds-deep-v1,
     config_matrix_id: config-impact-expanded-v1, held_out_policy_id: null,
     browser_case_policy_id: browser-declared-with-artifacts-v1, resource_policy_id: resources-deep-v1,
     outcome_reduction_profile_id: profile-selected-v1}
  - {tier_id: PROMOTION, tier_version: tier-promotion-v1, suite_selection: MILESTONE_PROFILE,
     suite_ids: [], impact_closure: REACHABLE_FIXED_POINT, seed_matrix_id: seeds-promotion-v1,
     config_matrix_id: config-milestone-v1, held_out_policy_id: held-out-milestone-v1,
     browser_case_policy_id: browser-milestone-required-v1, resource_policy_id: resources-promotion-v1,
     outcome_reduction_profile_id: milestone-required-only-v1}
```

Empty `suite_ids` means the declared `suite_selection` resolver supplies IDs and records them in the run manifest; it never means “run nothing.” Resource policies define timeouts, concurrency, memory/storage limits, retry prohibition/allowance, and artifact budgets. Seed/config/held-out/browser/reduction policy IDs MUST resolve before execution.
