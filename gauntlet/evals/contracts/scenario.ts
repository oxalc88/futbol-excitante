...
export interface RoutingFallbackScenario { id: string; kind: "routing_fallback"; input: { role: "critic" | "integration-reviewer"; failed_model: string; failure_class: GauntletFailureClass; builder_model: string }; expect: ScenarioExpectation }
...
export interface TimingConsistencyGateScenario { id: string; kind: "timing_consistency_gate"; input: { tracking_markers_match: boolean; clock_measurement_matches: boolean; latest_rows_present: boolean }; expect: ScenarioExpectation }
...
export interface PostAcceptanceContinuationGateScenario { id: string; kind: "post_acceptance_continuation_gate"; input: { acceptance_finalized: boolean; horizon_exhausted: boolean; next_objective?: string | null; stop_reason?: GauntletStopReason | null }; expect: ScenarioExpectation }
