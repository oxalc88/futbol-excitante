...
next_objective_id: BROWSER-5V5-ANTI-HUDDLE-DYNAMIC-EVIDENCE

best_known:
  commit: 455f4ec
  note: "BALL-SETTLED-REGIME-FIX accepted (Horizon v24 2/6) — settled balls now integrate applied impulses (same-tick, single regime re-entry transition per accepted thresholds; SETTLED_IMPULSE_WAKE_SPEED provisional from GROUND_SETTLE_SPEED; +37/−1 in ball-system.ts, the only core change this horizon). Flood stays closed (0/600 + 1/1200 pitch contacts); no teleport (≤ velocity×dt/tick); two-run byte-identical. Evidence trajectory.json SHA-256 0614dc0d… (3 runs). 23 discriminating guards (10/23 fail stashed). Honest pin regeneration, accepted bytes untouched. Critic+integration ACCEPT first pass (glm5.3-flash). Audit PASS 20/20. Horizon v24 2/6 — next: BROWSER-5V5-ANTI-HUDDLE-DYNAMIC-EVIDENCE (ball now visibly travels)."
...
selection_note: "Horizon v24 (anti-huddle-organic-pass-and-node-gate) ACTIVE 2/6 — created 2026-09-04 after horizon v23 exhaustion (5/5) and verified remote durability of the final acceptance commit d56ccad (push ebcfc5f..d56ccad), then AMENDED after the 1/6 acceptance (unsafe_due_to_new_defect). (1) 5V5-KICKOFF-ANTI-HUDDLE ACCEPTED (candidate 47bb0db, acceptance 25c0e13): kickoff freeze + nearest-only chase + fixed homes + one presser + cover; MULTI_TICK 1800-tick/30 s + discriminating stashed control; critic+integration ACCEPT first pass glm5.3-flash, audit PASS 20/20. (2) BALL-SETTLED-REGIME-FIX ACCEPTED (candidate 455f4ec): settled balls integrate applied impulses — settled→ground-roll wake after the accepted lift-off check (provisional SETTLED_IMPULSE_WAKE_SPEED from GROUND_SETTLE_SPEED; +37/−1 in ball-system.ts, the only core change this horizon); flood stays closed; no teleport; two-run byte-identical; 23 discriminating guards; honest pin regeneration with accepted bytes untouched; critic+integration ACCEPT first pass, audit PASS 20/20. Remaining four: (3) BROWSER-5V5-ANTI-HUDDLE-DYNAMIC-EVIDENCE — event-centered semantic frames + optional 30 s organic clip of kickoff freeze → first touch → spread → presser+cover → organic pass, now with the ball visibly traveling (builder-gameplay); (4) SMALL-SIDED-ORGANIC-PASS-FLOW-CLOSURE — scanner/reducer re-run over the anti-huddle + post-fix matches, honest coherent_match_sources, bundle supersession (builder-structured); (5) NODE-GATE-REGRESSION-TRIAGE — honest repair of the 6 pre-existing node failures still reproducing (compare-foundation ×2, nondeterminism-canary ×2, match-lifecycle determinism, playable-1v1-re-evaluation, COHERENT-EVIDENCE-RERUN-binding, difficulty-capture — match-set-piece passed in recent sweeps); reconcile OPEN inbox records once CI processes the pushes (builder-structured); (6) CAPTURE-HYGIENE-ENFORCEMENT — ordinary suites never write docs/screenshots/**, hygiene guard (builder-structured). GK/regulation/full-match/perceptual-rubric/networked/PES-fidelity remain deferred."
```
