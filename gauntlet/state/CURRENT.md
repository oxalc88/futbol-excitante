...
next_objective_id: SMALL-SIDED-HUMAN-ACTION-READABILITY-OBSERVABILITY

best_known:
  commit: b72ad12
  note: "SMALL-SIDED-CONTINUOUS-DUEL-AND-SHOT-CLOSURE accepted — SHOT_TO_RESULT localizes organically (7/8 present; PHYSICAL_DUEL honestly insufficient_context, orchestrator-signed partial closure) via a deterministic ball-system oscillation fix + controlledPlayerId passthrough; milestone bundle 16 runs / 14 sources. Horizon v22 1/5."
...
selection_note: "Horizon v22 (continuous-match-closure-and-small-sided-hardening) 1/5: SMALL-SIDED-CONTINUOUS-DUEL-AND-SHOT-CLOSURE accepted — SHOT_TO_RESULT now localizes ORGANICALLY (scanner 7 present / 0 not_observed / 1 insufficient_context on a 600-tick coherent 5v5 CPU-vs-CPU match) after a deterministic ball-system oscillation fix (MIN_LIFT_OFF_VELOCITY=0.5, POST_BOUNCE_ABSORB_THRESHOLD=1.0, ground-roll vz clamp; 511→1 pitch-contacts, 2v2 perf restored) + headless controlledPlayerId passthrough; PHYSICAL_DUEL honestly insufficient_context (input-rejection indicative structurally unreachable in organic CPU play; orchestrator-signed partial closure; milestone retains driven-fixture PASS). Regenerated BATCH-2-RERUN (SHOT_TO_RESULT genuine FAIL→PASS) + BATCH-5 index + PRESS trajectory under the new engine; corrected milestone playtest record (entry/exit prereq fields); bundle superseded to 16 runs / 14 sources (CDS source entry with critic+integration ACCEPT). 2 critic RETRY loops + 2 integration REJECT loops, all resolved (evidence regen, oscillation root-cause, trajectory authenticity, 2v2 forks-pool exit-0 via per-test file split). Critic ACCEPT + integration ACCEPT at b72ad12. Next: SMALL-SIDED-HUMAN-ACTION-READABILITY-OBSERVABILITY (builder-gameplay). GK/regulation/full-match deferred; no invented rubric/PES envelopes; no PROMOTION overclaim."
```
