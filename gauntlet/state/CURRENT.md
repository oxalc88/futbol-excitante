...
next_objective_id: 5V5-KICKOFF-ANTI-HUDDLE
...
selection_note: "Horizon v24 (anti-huddle-organic-pass-and-node-gate) ACTIVE 0/5 — created 2026-09-04 after horizon v23 exhaustion (5/5) and verified remote durability of the final acceptance commit d56ccad (origin/main contains it; push ebcfc5f..d56ccad). Reassessment inputs: the human-directed gameplay priority (stop the 5v5 ball huddle), the 9 node-gate failures verified to reproduce at the last remote-durable acceptance a46718c (worktree run 2026-09-04: 6 failed files / 8 failed tests in the targeted set — stale durable eval artifacts vs live registries, determinism canaries, match-lifecycle/set-piece, compare-foundation, COHERENT-EVIDENCE binding, difficulty-capture), the builder-disclosed capture-hygiene violation (ordinary suites rewrite docs/screenshots evidence), and the small-sided bounds (no GK/regulation/full-match/PES envelopes). Five objectives: (1) 5V5-KICKOFF-ANTI-HUDDLE — nearest-only chase + fixed kickoff homes + kickoff freeze until first touch + one presser + cover so organic pass fires, MULTI_TICK >=1800-tick (30 s) coherent 5v5 trajectory, not an 8-tick fixture (builder-gameplay); (2) BROWSER-5V5-ANTI-HUDDLE-DYNAMIC-EVIDENCE — event-centered semantic frames + optional 30 s organic clip of the same arc (builder-gameplay); (3) SMALL-SIDED-ORGANIC-PASS-FLOW-CLOSURE — scanner/reducer re-run over the anti-huddle matches, honest coherent_match_sources, bundle supersession (builder-structured); (4) NODE-GATE-REGRESSION-TRIAGE — honest repair of the 9 pre-existing node failures so the node gate exits 0; reconcile OPEN inbox records once CI processes the d56ccad push (builder-structured); (5) CAPTURE-HYGIENE-ENFORCEMENT — ordinary suites never write docs/screenshots/**, hygiene guard (builder-structured). GK/regulation/full-match/perceptual-rubric/networked/PES-fidelity remain deferred."
```
