...
next_objective_id: BALL-SETTLED-REGIME-FIX

best_known:
  commit: 47bb0db
  note: "5V5-KICKOFF-ANTI-HUDDLE accepted (Horizon v24 1/6) — kickoff freeze to first touch (0.000 m frozen displacement, single taker, first touch tick 18), nearest-only chase + fixed homes (huddleTicks 0 vs 352 stashed; max 2 same-team bodies within 5 m), one presser + cover via shared designatePresser, organic passes fire (11 events / 119.99 m ball travel in the 1800-tick/30 s flowing run; stashed control 0 passes — discriminating). Adapter/team-decision layer only, core+contracts byte-identical, stash-identity + pinned baselines byte-for-byte, two-run determinism. Evidence trajectory.json SHA-256 29105045… (3600 rows, 3 runs). Critic ACCEPT first pass (independent re-execution, identical hash chain); integration ACCEPT first pass (101 neighbor tests green). Audit PASS 20/20 MULTI_TICK. Horizon amended: BALL-SETTLED-REGIME-FIX inserted (settled-ball regime applies no physics — blocks honest organic-pass visuals)."
...
selection_note: "Horizon v24 (anti-huddle-organic-pass-and-node-gate) ACTIVE 1/6 — created 2026-09-04 after horizon v23 exhaustion (5/5) and verified remote durability of the final acceptance commit d56ccad (origin/main contains it; push ebcfc5f..d56ccad). Reassessment inputs: the human-directed gameplay priority (stop the 5v5 ball huddle), the 9 node-gate failures verified to reproduce at the last remote-durable acceptance a46718c (worktree run 2026-09-04: 6 failed files / 8 failed tests in the targeted set — stale durable eval artifacts vs live registries, determinism canaries, match-lifecycle/set-piece, compare-foundation, COHERENT-EVIDENCE binding, difficulty-capture), the builder-disclosed capture-hygiene violation (ordinary suites rewrite docs/screenshots evidence), and the small-sided bounds (no GK/regulation/full-match/PES envelopes). Six objectives after the 2026-09-04 amendment: (1) 5V5-KICKOFF-ANTI-HUDDLE ACCEPTED (nearest-only chase + fixed kickoff homes + kickoff freeze until first touch + one presser + cover so organic pass fires; MULTI_TICK 1800-tick/30 s trajectory + discriminating stashed control; candidate 47bb0db, critic+integration ACCEPT first pass glm5.3-flash, audit PASS 20/20); (2) BALL-SETTLED-REGIME-FIX — horizon amendment (unsafe_due_to_new_defect, builder-disclosed + evidence-confirmed): settled-ball regime applies no physics, so touch/pass events fire with zero ball travel; core fix with byte-protection of the accepted ball-oscillation fix and honest regeneration of any legitimately changed accepted pins (builder-gameplay); (3) BROWSER-5V5-ANTI-HUDDLE-DYNAMIC-EVIDENCE — event-centered semantic frames + optional 30 s organic clip of the arc (builder-gameplay, now depends on the ball fix so passes visibly travel); (4) SMALL-SIDED-ORGANIC-PASS-FLOW-CLOSURE — scanner/reducer re-run over the anti-huddle matches, honest coherent_match_sources, bundle supersession (builder-structured); (5) NODE-GATE-REGRESSION-TRIAGE — honest repair of the 9 pre-existing node failures so the node gate exits 0; reconcile OPEN inbox records once CI processes the pushes (builder-structured); (6) CAPTURE-HYGIENE-ENFORCEMENT — ordinary suites never write docs/screenshots/**, hygiene guard (builder-structured). GK/regulation/full-match/perceptual-rubric/networked/PES-fidelity remain deferred."
```
