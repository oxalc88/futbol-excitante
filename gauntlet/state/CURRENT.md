...
next_objective_id: GK-BROWSER-DYNAMIC-EVIDENCE


best_known:
  commit: 40aa0da
  note: "GK-5V5-ADAPTER-BEHAVIOR accepted (Horizon v26 2/4) — designated-keeper behavior in the 5v5 CPU-vs-CPU match at the adapter/team-decision layer (core byte-identical): goalkeeper-role.ts designates one existing body per team (no new world bodies); keeper holds its goal arc with bounded drift (gk-small-sided-v1), is never chaser/presser/cover/taker (anti-huddle contract inherited), and performs save/claim reactions ONLY via tick-indexed InputFrames (ball independent, no parenting/teleport). gkBehavior:false kill switch strictly opt-in and reproduces 91ff0be per-tick hash chains byte-for-byte (stash-identity verified in a detached worktree; continuous stashed chain byte-equals the accepted anti-huddle pin). Evidence: trajectory.json SHA-256 ca9443a0… (4 runs: continuous live/stashed 1800 ticks + driven shot-fixture live/stashed 600 ticks; live replays identical; 4 completed save chains in the driven fixture — shot 361→contact 362 @1.08 m; organic continuous match armed 21 reactions / 0 completed chains — honestly disclosed, save evidence is driven-by-layout). Runner touches are default-false evidence wiring (gkBehavior into CpuObservation); no football state owned by the runner. Tests 51 new + accepted pins 64/64; typecheck 0. Builder reroute disclosed: qwen3.8-flash hit its monthly token cap mid-task at ~80% completion; deepseek-v4-flash completed and re-verified (critic + integration independently re-verified line-by-line). Critic (glm5.3-flash) ACCEPT first pass; integration ACCEPT first pass (115/115). Audit PASS, MULTI_TICK."
...
selection_note: "Horizon v26 (goalkeeper-arc-small-sided) ACTIVE 2/4 - created 2026-09-05 after Horizon v25 COMPLETE 4/4 (remote-durable at 5829156). (1) GK-SPEC-SUITE-CONTRACTS ACCEPTED (candidate 0f43188): dedicated small-sided GK spec + versioned `goalkeepers` suite contracts; no behavior; no gameplay PASS. (2) GK-5V5-ADAPTER-BEHAVIOR ACCEPTED (candidate 40aa0da): designated-keeper hold/save at the adapter layer (gk-small-sided-v1; ball independent via InputFrames; gkBehavior:false stash-identity to 91ff0be; 4-run MULTI_TICK trajectory ca9443a0…; 4 driven save chains; organic run 21 reactions / 0 completed chains disclosed; builder rerouted qwen3.8-flash→deepseek-v4-flash mid-task on quota cap); critic+integration ACCEPT first pass; core byte-identical. Remaining: (3) GK-BROWSER-DYNAMIC-EVIDENCE - browser DYNAMIC_VISUAL of GK positioning + save/claim (enable gkBehavior in the browser composition root; builder-gameplay); (4) GK-SUITE-ORGANIC-STATE - honest organic GK-suite state re-run (builder-structured). GK beyond small-sided, regulation rules, full-match ecology, perceptual-rubric/networked/PES-fidelity remain deferred."
```
