...
next_objective_id: BROWSER-SWITCH-INDICATOR-BASELINE-FIX

best_known:
  commit: cbd1878
  note: "SMALL-SIDED-HUMAN-ACTION-READABILITY-OBSERVABILITY accepted — human-driven PASS/SHOT (J/L) in the 5v5 human-vs-CPU browser match now has 5 event-centered before/event/after frames byte-bound to the exact input tick/frame (pass 256→257 PASS_BIT=2, shot 286→287 SHOT_BIT=4), single-source-of-truth tick offsets; critic ACCEPT (after 3 RETRY loops) + integration ACCEPT; milestone bundle 16 runs / 15 sources. Horizon v22 2/5."
...
selection_note: "Horizon v22 (continuous-match-closure-and-small-sided-hardening) 2/5: SMALL-SIDED-HUMAN-ACTION-READABILITY-OBSERVABILITY accepted — human-driven discrete actions (PASS via J, SHOT via L) in the 5v5 human-vs-CPU browser match now observable as 5 event-centered before/event/after DYNAMIC_VISUAL frames byte-bound to the exact input tick and input frame (pass event 257 ← input frame 256 PASS_BIT=2; shot event 287 ← input frame 286 SHOT_BIT=4; same-tick resolution policy documented). Single source of truth for frame-tick offsets (eval/scenarios/frame-tick-offsets.ts, PASS_BEFORE_OFFSET=-10 / PASS_AFTER_OFFSET=12 / SHOT_AFTER_OFFSET=12) imported by capture test + binding test so metadata cannot drift from capture; binding guard requires pairwise-unique SHA-256 per semantic label (no carve-out), non-vacuous PASS_BIT/SHOT_BIT assertions with negative controls. NO readability PASS / invented rubric (observability evidence for reviewer judgment only). 3 critic RETRY loops (stale capture 843cf468 triple-hash → recapture; leftover carve-out + orphan + vacuous bindings; tick-metadata drift vs capture) — all resolved; integration ACCEPT first pass (457 unit + 47 browser neighbors, zero src/contracts/spec change, typecheck errors pre-existing). Milestone bundle superseded to 16 runs / 15 sources. Next: BROWSER-SWITCH-INDICATOR-BASELINE-FIX (builder-gameplay). GK/regulation/full-match deferred; no invented rubric/PES envelopes; no PROMOTION overclaim."
```
