...
next_objective_id: SMALL-SIDED-LADDER-MENU-COMPLETION

best_known:
  commit: c607e56
  note: "BROWSER-SWITCH-INDICATOR-BASELINE-FIX accepted — pre-existing baseline failures (player-indicator INDICATOR-002; player-switch SWITCH-004/005/006) fixed by reconciling the legacy browser helpers + main.ts real-time loop to the core-native single-switch contract (SWITCH_PLAYER_BIT resolved in sim.step(); manual setControlledPlayer removed; core untouched); critic ACCEPT (after 1 RETRY, stale JSDoc) + integration ACCEPT; milestone bundle 16 runs / 16 sources. Horizon v22 3/5."
...
selection_note: "Horizon v22 (continuous-match-closure-and-small-sided-hardening) 3/5: BROWSER-SWITCH-INDICATOR-BASELINE-FIX accepted — the pre-existing baseline failures surfaced in v21 (player-indicator INDICATOR-002; player-switch SWITCH-004/005/006) are fixed by reconciling the legacy browser test helpers and main.ts's real-time loop to the core-native single-switch contract: SWITCH_PLAYER_BIT is resolved inside sim.step() (mutating controlAssignments + emitting slot-switch), and the manual setControlledPlayer/nextEligiblePlayer double-switch path is removed (main.ts −21 lines; both runWithCpu helpers de-switched). Deterministic core byte-identical (git diff src/simulation/ empty). Discriminating guards: SWITCH-GUARD / INDICATOR-GUARD positive (single Tab → exactly one switch + one slot-switch event; marker follows) with negative controls (no bit → no switch/event, simulates stashed core); SWITCH-001/002/003 + INDICATOR-001/003/004/005 semantics preserved. 8/8 + 7/7 + 1 capture exit 0; 150 browser neighbors + 232 core units green; audit PASS; RESULT.md claims_not_made. Critic RETRY ×1 (stale JSDoc claiming removed manual switch) — fixed comment-only; integration ACCEPT first pass. Milestone bundle superseded to 16 runs / 16 sources. Next: SMALL-SIDED-LADDER-MENU-COMPLETION (builder-gameplay). GK/regulation/full-match deferred; no invented rubric/PES envelopes; no PROMOTION overclaim."
```
