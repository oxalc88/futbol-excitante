...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 5f29f2f candidate(RULES-FACTS-DEPTH-CONFORMANCE)
- notes: Horizon v30 1/4. The remaining non-corner NOT_EVALUATED rules criteria evaluated from existing serialized facts — NO change to the serializeRestartFacts injection (the facts already existed). TEN criteria upgraded NOT_EVALUATED→PASS: THROW-IN-PLACEMENT (throwPosition == paired touchline-exit ballPosition §6.3), THROW-IN-SERVE (chest height z=1.5, upward vz, distinct into-play target, unit direction §6.4), THROW-IN-TIMER-FREEZE + GOAL-KICK-TIMER-FREEZE (no decrement during restart phases §11), GOAL-KICK-PLACEMENT (goal-area spot on the exit side §7.3), KICKOFF-FIRST-TOUCH (taker's first touch §9.2), SCORING-GOAL-PHASE (playing→goal→playing §10.2), TIMER-DECREMENT (only during playing §11), TIMER-HALFTIME + TIMER-FULLTIME — genuine timer-driven transitions observed on ONE NEW driven full-match timing fixture (eval/scenarios/5v5-full-match-timing.v1.json, versioned 5v5-full-match-timing-v1, matchDurationTicks 240 / durationTicks 800): half 1 decrements 240→1 during playing then core-driven playing→halftime with the §9.4 60-tick break countdown loaded in-step; half 2 a literal 1→0 zero-crossing at t=660→661 with 140 fulltime ticks at timer=0; the fulltime oracle explicitly REJECTS a runner-stamped terminal fulltime label (stays NOT_EVALUATED on the 1800-tick fixtures — critic reproduced). Verdict table now 17 PASS / 2 BLOCKED_MISSING_REFERENCE / 6 NOT_EVALUATED / 0 FAIL. The 3 anti-huddle restart-behavior criteria (RESTART-FREEZE-UNTIL-FIRST-TOUCH, RESTART-NEAREST-ONLY, RESTART-REARM) stay honestly NOT_EVALUATED — the committed streams carry no adapter-designation facts (verified absent by the critic's event-kind scan) and the set-piece ball is served immediately, not held frozen; nothing synthesized. Corner cluster out of scope (owned by CORNER-DRIVEN-CONFORMANCE). Zero core change (git diff src/ EMPTY; headless-match.ts EMPTY — injection untouched). Changes: 10 additive protected oracle checks (rules-restart 5 + rules-phase 5) wired additively (the only removed lines: 10 empty binding arrays now bound + 2 payload type-line extensions; existing award/freeze oracles verdict-identical — pinned RULES-SUITE-STATE table stays green); rules-oracle +29 tests (FAIL-direction mutations included); restart-rules-serialization +1 full-match guard; RULES-SUITE-REGISTRATION-binding +10 criteria; NEW rules-facts-depth-binding 8; NEW capture script + timing scenario + evidence dir. Evidence: trajectory 6 runs (3 live + 3 stashed; state-hash chains identical per pair); record_sha256 ebf9083136b051d50fcf3f4efbcc55eeb8f53fda9d43cd36b8e08d0874852e61 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary-mode runs byte-identical); audit PASS MULTI_TICK (16/4/0). Tests: rules gate 121 (integration re-ran 149 across the wider gate) + neighbors 231/231 + foundation/provenance/hygiene 81/81 + GK/stateHash pins 28/28; typecheck 0. Critic ACCEPT first pass (re-ran the production runner + evaluator on all three live streams, 0 mismatches across 3×25 verdicts; verified timer evidence genuine); integration ACCEPT first pass (architecture 27/27; silent-consumer hunt over bindings/invariant-definitions/wire consumers clean — payload type extensions erased at runtime; scenario registry-clean). Reviewer non-binding notes recorded for the next rules objective: checkKickoffFirstTouch doc-comment/implementation gap on keeper taker exclusion (unreachable in the registered binding context; no verdict affected); tighten the inherited claims wording to "no suite-level PASS" at the next natural edit. claims_not_made: no suite-level PASS claim / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented envelope (1.5/5.5/9.16/60-tick are the spec's own match-rules-v1 VERSIONED_PROVISIONAL parameters; 0.2 m placement tolerance is evaluator-side comparison slack) / blocked references stay blocked / no criterion upgraded beyond the executed evaluator / no corner claim / no anti-huddle restart-behavior claim / no core change.

## Iteration 177 — 2026-09-06

- objective_id: CORNER-DRIVEN-CONFORMANCE
- builder: builder-structured / deepseek-v4-flash
- critic: critic / glm5.3-flash — RETRY→ACCEPT (3 fixes: sequence.json path bindings, DYNAMIC_VISUAL-class audit incl. semantic visual sequence, report hygiene)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass on the repaired candidate)
- result: accepted
- commits: c07cebb candidate(CORNER-DRIVEN-CONFORMANCE)
- notes: Horizon v30 2/4. A GENUINE driven corner completes the per-restart conformance set. Mechanism: 5v5-corner-driven-v1 = the accepted 5v5-restart-arc layout with TWO driven initial-state changes only (ball (51.5, 12, 0.11) inside the +x goal-line span outside the posts; team-b defender player-9 at (47, 12) between ball and field centre) — empty inputProgram/scheduledEvents, the accepted driven-fixture discipline; the adapter drives player-9 to the ball, its touch sends the ball over the +x goal line, and because team-b (the +x defending team) was last touch the CORE's own §8.1 rules award a corner to team-a and execute it (boundary tick 71 ball-out-of-play goalIndex 0 lastTouchRef pass-48-2 → team-b; execution tick 131 corner-kick-executed teamId team-a cornerPosition (52.5, 34) = §8.2 nearest corner flag, cross toward (44.5, 0)). Robust across seeds 7/123/999 and defender x ∈ {46,47,48} (critic AND integration independently reproduced). Outcomes: CORNER-KICK-AWARD + CORNER-KICK-PLACEMENT (new protected oracle checkCornerKickPlacement, CORNER_FLAG_Y from §8.2) + CORNER-KICK-TIMER-FREEZE (new protected oracle checkCornerKickTimerFreeze) all NOT_EVALUATED→PASS; CORNER-KICK-CROSS stays BLOCKED_MISSING_REFERENCE (never invented); goal-kick neighbour is a discriminator (0 corners). Suite now 20 PASS / 2 BLOCKED / 3 NOT_EVALUATED (anti-huddle restart-behavior, honestly unanswerable) / 0 FAIL. DYNAMIC_VISUAL evidence (strictest named class): 4 event-centered Chromium frames (before@60 → award@71 → set-piece@105 → execution@137, distinct SHAs, corner-aimed camera disclosed) with sequence.json path bindings written by the browser test source; audit PASS at --class DYNAMIC_VISUAL including semantic-visual-sequence. CRITIC RETRY: the first candidate ran the audit at MULTI_TICK while naming DYNAMIC_VISUAL frames — the critic reproduced the strictest-class audit FAIL ("semantic visual sequence": frames lacked the repo-wide path binding) and enforced the evidence contract's no-downgrade rule; fixes (path bindings in the writer + re-capture, DYNAMIC_VISUAL-class audit persisted, hygiene: binding count 8→7 with arithmetic restated 66+17+25+4+10+8+7=137, phantom RECORD entry removed, doc/evidence typo fixed) verified with the pinned record BYTE-UNCHANGED. record_sha256 21e3aa08be951f1416bb5258aef666218c1533716cfea3eea40a71a782b279b1 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary-mode runs byte-identical; docs/ byte-identical on ordinary runs). Tests: rules gate 137/137 + neighbors 231/231 + foundation/provenance/hygiene 81/81 + browser 2/2 in Chromium; typecheck 0; zero core/spec change. Integration ACCEPT first pass on the repaired candidate (driven corner reproduced across all seed/position variants; frame SHAs proven real-capture). claims_not_made: no suite-level PASS claim / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented envelope / CORNER-KICK-CROSS stays BLOCKED_MISSING_REFERENCE / the corner is DRIVEN not organic (disclosed everywhere) / no forced or synthesized events / no criterion upgraded beyond the executed evaluator / no core or spec change.

## Iteration 178 — 2026-09-06

- objective_id: GK-CORE-OWNED-ARC-FIX
- builder: builder-gameplay / deepseek-v4-flash (reroute — qwen3.8-flash quota-exhausted until 2026-10-01; first launch attempt 402'd, relaunched with the explicit reroute)
- critic: critic / glm5.3-flash — ACCEPT (first pass)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: e687fa9 candidate(GK-CORE-OWNED-ARC-FIX)
- notes: Horizon v30 3/4. The core-owned team-a keeper off-arc drift (disclosed in the accepted GK-GOALLINE-BOUNDS-RESIDUAL) root-caused and fixed at the adapter layer. ROOT CAUSE (critic reproduced exactly: single-tick jump 24.6711 m at tick 451, velocity forced 0): the core-owned post-goal reset — the core's applyGoalReset re-places EVERY body at its scenario kickoff home after the tick-391 goal — strands the team-a keeper because its scenario kickoff home (-30, -10) sits 24.62 m off its own goal arc centre (-52.5, 0), while team-b's keeper home is on-arc (0.32 m) and holds; the keeper re-transits at the locomotion cap but cannot recover before the run ends (~9.47 m off-arc at tick 600); on every off-arc tick chaser=none (keeperDesignatedChaserTicks=0) — NOT a chase-arbitration bug; the legacy lifecycle masked the reset by overriding the phase every tick. FIX (change confined to eval/runners/headless-match.ts, 92 lines; zero src/ change): pure deterministic `rehomeKeeperToArc(scenario)` re-homes a designated keeper whose kickoff home is off its own goal arc onto that arc (keeperArcSetPoint = goal-line centre, ±2.5 lateral clamp, VERSIONED gk-small-sided-v1 geometry — no invented constant), new scenario copy only when changed; gated `rehomeKeeper ?? (gkBehavior && lifecyclePhaseSync === "core-owned")` with a rehomeKeeper:false opt-out; the world is created from the re-homed scenario so the core's own reset restores on-arc positions (the fix works through the core's existing machinery). AFTER: GK-POSITIONING-HOLD / GK-NO-FIELD-CHASE PASS (maxDist 2.50 m, offArc 0/600, onArcRatio 1.00; the reset still executes — 2.9 m jump — but lands on-arc). SILENT-CONSUMER HUNT (LIFECYCLE-MIGRATION discipline, re-verified independently by critic AND integration): every accepted-pin producer threads explicit legacy or gkBehavior:false → byte-identical; stash identity verify-gk-stash 91ff0be PASS 4/4; exactly TWO fresh-run consumers change at HEAD, both targeting the fixed defect and both regenerable via rehomeKeeper:false (capture-goalline-bounds-residual + its guard, which still passes 7/7; the LIFECYCLE-MIGRATION probe's core-owned gk arms — binding reads persisted artifacts only); browser composition root has zero references to the runner helper (browser keeper behavior adapter-owned, human modes untouched). Evidence: record_sha256 92e4c4a3a59d45796173e652e03833acf529182db25a99d3626fd93bcd86cc7d (no wall-clock field — the fresh ordinary-mode artifact is byte-identical to the durable record; two consecutive ordinary runs byte-identical; docs/ byte-identical on ordinary runs); audit PASS HEADLESS. Tests: guard 6/6 + GK families 83/83 + stateHash pins 50/50 + integration/runner neighbors 117/117 (critic re-ran 324 tests + stash verification); typecheck 0. Critic ACCEPT first pass; integration ACCEPT first pass. claims_not_made: no suite-level PASS claim (goalkeepers suite re-published under core-owned by the next objective) / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented constant / no oracle weakening (a stranded keeper still FAILs — guard-proven) / no core, adapters, scenario, or spec change / accepted legacy-produced GK suite records byte-untouched / GK beyond small-sided stays deferred.

## Iteration 179 — 2026-09-06
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: de9fd3b candidate(GK-SUITE-CORE-OWNED-STATE)
- notes: Horizon v30 4/4 (horizon COMPLETE). The goalkeepers suite re-published under the core-owned lifecycle (BOOKKEEPING; zero source change — verified empty by builder, critic, AND integration; v27 records byte-untouched). Core-owned verdict table (re-home live): 8 PASS (POSITIONING-HOLD, NO-FIELD-CHASE, SAVE-CLAIM, ROLE-DESIGNATION, DISTRIBUTION-NO-OMNISCIENCE, COMMON-FINITE, COMMON-REFERENCES, COMMON-BOUNDS) / 3 NOT_EVALUATED (COMMON-DETERMINISTIC single-run; SAVE-CLAIM organic 0 save chains; DISTRIBUTION fixture 0 releases) / 1 BLOCKED_MISSING_REFERENCE (GK-*-REF) / 1 NEEDS_PERCEPTUAL_REVIEW (GK-*-VIS) / 0 FAIL. The ONE true verdict change vs the accepted v27 legacy table: COMMON-BOUNDS FAIL→PASS (legacy phase-sync froze restarts so the ball escaped the pitch without a restart — bodies 59.47-61.18 m; under core-owned the restart machinery runs and the accepted GK-GOALLINE-BOUNDS-RESIDUAL goal-mouth bound 56.5 m resolves the team-b keeper goal-mouth position; max|player.x| 52.500/52.531, max|ball.x| 52.629). Two source-flip disclosures (verdicts unchanged): DISTRIBUTION observation source driven(2 releases)→organic (8 releases to observed player-9, fixture 0); POSITIONING/NO-FIELD-CHASE run source legacy→core-owned — PASS only because the re-home is live (critic verified via rehomeKeeper:false re-runs: both FAIL, 24.622 m / 47.877 m off-arc). Forwarded disclosure: capture-goalline-bounds-residual.ts + the LIFECYCLE-MIGRATION probe's core-owned gk arms re-run WITH the re-home at HEAD; accepted bytes regenerate only via rehomeKeeper:false. Evidence: record_sha256 5cd1c80879d400c2dabfc0ba6d9b195447fc9476f7b6ebb5329877dd2a69204a (no wall-clock field — only candidate_commit = HEAD; two consecutive ordinary-mode runs byte-identical to each other AND to the durable record, 1ef55d8b…; docs/ byte-identical on ordinary runs); audit PASS BOOKKEEPING. Tests: binding 11/11 (incl. an 87 s physical reproduction of both keeper runs through the production runner + evaluator) + GK families 58/58 + stateHash pins 40/40 + rules gate 88/88 + evaluator/hygiene 153/153 + stash identity 4/4; typecheck 0. Critic ACCEPT first pass (own independent script re-derived the table with zero mismatches; programmatic v27 diff; empty-chain facts reproduced by its own chain checks; rehomeKeeper:false dependency verified); integration ACCEPT first pass (batteries re-run green; live re-run re-emits exactly the recorded per-run verdicts, 0 mismatches; scope = exactly horizon item 4/4, no creep). claims_not_made: no suite-level PASS claim (negative-control binding) / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented reference envelope (GK reference keys stay BLOCKED) / no criterion upgraded beyond the executed evaluator / no oracle/catalog/invariant/observation/scenario/spec change / no accepted record mutation / zero gameplay change.

## Iteration 180 — 2026-09-06
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 0d7d76d candidate(RESTART-DESIGNATION-FACTS-CONFORMANCE)
- notes: Horizon v31 1/4. The adapter restart-window designation facts ARE runner-observable, and the gated serializeRestartFacts injection now carries them — closing the 3 anti-huddle restart-behavior criteria through the rules suite. MECHANISM (gk-role precedent extended): restart-designation events computed STRICTLY post-loop from the SAME production source of truth the adapters act on — assignChaseRoles (cpu-adapter.ts:1724-1753, called per tick at :2695) + committed coreMatchPhases + ball lastTouchRef — carrying ballUntouched, designated taker, per-team designated chaser, per-body window anchor, baselineTouchRef/re-armed state; the runner propagates the ACTUAL adapter designation, owns no football state. Empirical: browserParity arc run 8 untouched windows incl. 6 re-armed post-goal; non-browserParity 2 (no re-arm); critic re-computed assignChaseRoles from a fresh 1800-tick arc run — 0/3600 team-tick mismatches. OUTCOMES: RESTART-FREEZE-UNTIL-FIRST-TOUCH PASS, RESTART-NEAREST-ONLY PASS, RESTART-REARM PASS (arc-live post-goal + fullmatch-live halftime; honestly NOT_EVALUATED on throwin-live — no reset); composition over the 3 browserParity live runs 20 PASS / 2 BLOCKED / 3 NOT_EVALUATED (corner cluster out of scope, owned by the accepted CORNER-DRIVEN-CONFORMANCE streams) / 0 FAIL. FOLD-IN fixes (both reviewer non-binding notes): checkKickoffFirstTouch now EXCLUDES the keeper from taker selection (implementation aligned to the doc comment after spec §12.1 verified to mandate it; reads the injected gk-role designation only — a no-op without gk-role events; KICKOFF-FIRST-TOUCH stays PASS everywhere previously PASS, the accepted RULES-FACTS-DEPTH-CONFORMANCE pin preserved); claims wording tightened to the accurate "no suite-level PASS" phrasing. INJECTION SAFETY under the enlarged surface (re-verified by critic AND integration): role functions are pure readers; gate-off byte-identity + live/stashed chain identity re-verified (3 stashed controls identical, 0 injected facts); the mirror's one-step ref offset is conservative — can only shorten windows or false-fail, never fabricate a PASS; keeper-exclusion gating structural (gk-role events only under gkBehavior && keeperRoles !== undefined; no producer combines gkBehavior with serializeRestartFacts — a no-op on ungated rules runs). Falsifiers both-direction: wrong-taker injection FAILs freeze (critic adversarially confirmed), clump FAILs nearest-only, missed re-arm FAILs rearm. Evidence: trajectory 6 runs (3 live + 3 stashed); record_sha256 271b1526592cc13e3792bee42f2544379e7dea16de9571b43113b32b57e7fc56 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical; docs/ byte-identical on ordinary runs); audit PASS MULTI_TICK. Tests: rules gate 150 (integration re-ran 185 wider) + GK/stateHash pins 34-109 + registry/provenance 102-123 + neighbors 80/80 + hygiene 16/16; typecheck 0. Critic ACCEPT first pass (every battery personally executed; mechanism verified against production code); integration ACCEPT first pass (silent-consumer hunt over 5 producers + 5 test-file consumers; default-false path byte-identical). Critic non-binding observations recorded for future cleanups: dead chasers variable in checkRestartNearestOnly; the conservative mirror offset (disclosed, acceptable); the post-loop gated calls inflate the _keeperPressExclusions diagnostics counter (read by no gated evidence; future hygiene: never combine gated designation runs with GK counter reads in one process). claims_not_made: no suite-level PASS claim (accurate wording used) / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented constant / corner-cross + goal-kick-distribution stay BLOCKED_MISSING_REFERENCE / no forced or synthesized events / no criterion upgraded beyond the executed evaluator / no core or spec change.

## Iteration 181 — 2026-09-06
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 030c2e0 candidate(RULES-SUITE-STATE-RERUN)
- notes: Horizon v31 2/4. The rules suite's complete aggregate verdict state re-published across all evidence generations (BOOKKEEPING; zero source change — verified empty by builder, critic, AND integration; no accepted record mutated). AGGREGATE TABLE: 23 PASS / 2 BLOCKED_MISSING_REFERENCE / 0 NOT_EVALUATED / 0 FAIL over all 25 MATCH-* criteria (+ all 8 protected invariants PASS). EXACT DELTAS vs the RULES-FACTS-DEPTH-CONFORMANCE baseline (record ebf90831…, 17/2/6/0): 6 criteria NOT_EVALUATED→PASS with source-stream attribution — RESTART-FREEZE-UNTIL-FIRST-TOUCH + RESTART-NEAREST-ONLY + RESTART-REARM from the designation streams (REARM correctly absent from the no-reset throw-in stream); CORNER-KICK-AWARD + CORNER-KICK-PLACEMENT + CORNER-KICK-TIMER-FREEZE from rules-corner-live (discriminated by the NOT_EVALUATED goal-kick neighbour control); 17 PASS unchanged; 2 BLOCKED unchanged; 0 FAIL introduced. BASELINE-COUNT CORRECTION (disclosed in the record; verified by the critic against the accepted records): the horizon/task text attached the "20/2/3" figure to the RULES-FACTS-DEPTH baseline, but it is actually the RESTART-DESIGNATION-FACTS aggregate (271b1526…); the true RULES-FACTS-DEPTH baseline is 17/2/6 (ebf90831…) — both converge on the same 23/2/0/0 aggregate; the horizon v31 item-2 reason text corrected in place during this iteration's bookkeeping. DISCLOSURES (critic verified via runner code + a same-stream probe; integration via record reads): (a) anti-huddle criteria evaluated only on the browserParity designation streams — on non-browserParity gated streams the anti-huddle oracle result is a wiring/shape artifact (criterion_eligibility; parity shape at headless-match.ts:949, re-arm arming only under parity wiring :1144-1204); (b) the RULES-SUITE-STATE continuous-play corner baseline control (~149 s/run) not re-run within the capture budget — redundant (its only PASS verdicts OOP-DETECT/GOAL-DEVENT also PASS on the re-run streams; corner NOT_EVALUATED superseded by the driven corner PASS; published verdicts remain in record bae56e5a…). Evidence: record_sha256 36fc77e52909dbaceefa14927b37b8e533c248e451477fc579dc4952434979ef (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical AND byte-equal to the durable record 622f1008…; docs/ byte-identical on ordinary runs); audit PASS BOOKKEEPING. Tests: binding 9/9 + rules gate 166/166 (integration re-ran 179 wider) + neighbors 242/242 (one pre-existing vitest worker-RPC reporting timeout, all assertions green) + foundation/provenance/hygiene 81/81 + stateHash pins 69/69; typecheck 0. Critic ACCEPT first pass (own producer runs ×2; programmatic diff found exactly 6 changed criteria and no other delta; aggregate recomputed 25/25; eligibility verified from runner code + same-stream probe; the omitted control's verdict list confirmed aggregate-neutral); integration ACCEPT first pass (batteries re-run green; verify-acceptance-durability PASS at HEAD; silent-consumer hunt clean). claims_not_made: no suite-level PASS claim (the accurate phrasing; the 2 BLOCKED references keep the suite from a clean PASS) / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented envelope / blocked references stay blocked / no criterion upgraded beyond the executed evaluator / zero gameplay/source/contract/adapter/spec change.

## Iteration 182 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 72f52b2 candidate(BROWSER-FULL-MATCH-FLOW-EVIDENCE)
- notes: Horizon v31 3/4. The full-match lifecycle is now VISIBLE in the real browser app — the horizon's observable browser-facing capability (DYNAMIC_VISUAL, audited at the strictest named class per the CORNER-DRIVEN lesson). BEFORE: the DOM scoreboard showed a tick-derived MM:SS clock + a "1ST"/"2ND" indicator with a transient HALF TIME / FULL TIME overlay fading after ~1 s — the halftime-break countdown and the fulltime terminal state were NOT distinguishable. AFTER: a minimal OPT-IN renderer screen-space label (`RendererConfig.showMatchPhaseHud`, default FALSE in DEFAULT_RENDERER_CONFIG; only main.ts:584 opts in) renders the immutable snapshot's pre-existing matchPhase/matchTimer fields; formatPhaseLabel maps exactly the 8 core-owned MatchPhase values with an uppercase fallback; draw-only (own canvas/texture created once; the off-branch is the byte-equal original single renderer.render call). CAPTURED FRAMES (5 event-centered Chromium frames, path-bound in sequence.json written by the test source): kickoff (0, PLAYING/240) → first-half-late (299, PLAYING/1) → halftime-break (330, HALF TIME/30) → second-half-kickoff (380, PLAYING/220) → fulltime (660, FULL TIME/0), distinct SHAs. CORRESPONDENCE honestly disclosed: phase_sequence_matches=true re-derived live; a 1-tick offset TRACED (the headless runner seeds a non-decrementing kickoff tick 0 — headless 301/661 vs browser 300/660); NO per-tick hash-identity claim anywhere. The 240-tick-half scenario is fixture-driven and disclosed — no organic 90-minute claim. Zero simulation change (git diff src/simulation/ src/contracts/ EMPTY). Evidence: record_sha256 d02a8fa6de7b2daaa5d9360b27cbb8b36e8c0c5920a25cf14b4a00192cf403ba (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical; the WIP gate verified over the 956-file docs tree); audit PASS at --class DYNAMIC_VISUAL (semantic visual sequence + screenshot SHA uniqueness PASS). Tests: browser test 3/3 + keeper-visual-marker 4/4 with the PRE_CHANGE_BASELINE_SHA 05e40d0117…68bc EXACT + render-sensitive neighbors 24/24 + byte-identity guards 7/7 + menu/parity 27/27 + human modes 22/22 + THE ENTIRE BROWSER PROJECT re-run by integration: 48 files, 256 tests, 0 failures + node render-referencing guards 21/21 (incl. BUILD-VITE-RESOLVE-001 building the real app) + pnpm run build PASS; typecheck 0. Critic ACCEPT first pass (renderer-neutrality verified; all 5 PNGs visually inspected; the 1-tick offset traced to the runner source); integration ACCEPT first pass (the sole createPresentationSession call sites enumerated — test-bridge pass-through HUD-off, main.ts the only opt-in; presentation_authority PASS). Non-blocking observation recorded: the HUD ortho camera anchors at session creation with no resize re-anchoring (presentation-only, no outcome/guard impact). claims_not_made: no PES fidelity / no FOUNDATION_LAB_PASS / no suite-level PASS / no organic 90-minute claim / no per-tick browser-headless hash-identity claim (disclosed negative) / no football-outcome change (draw-only; simulation and contracts byte-identical) / no invented constants.

## Iteration 183 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: f3a50ee candidate(SUITE-DETERMINISTIC-TWO-RUN)
- notes: Horizon v31 4/4 (horizon COMPLETE). Two-run deterministic attestation for both registered suites so COMMON-DETERMINISTIC evaluates (BOOKKEEPING; zero source change — verified empty by builder, critic, AND integration; no accepted record mutated). MECHANISM: COMMON-DETERMINISTIC is a HARD_INVARIANT with no single-run oracle (deliberately excluded from CRITERION_TO_ORACLE and single-run overall reduction) — it resolves from a two-run comparison per GAMEPLAY_EVALUATION_SPEC §HARD_INVARIANT and the compare-foundation two-run shape: two runs of the same pinned run contract with identical per-tick stateHashes ⇒ PASS, divergence ⇒ FAIL with earliest_divergence_tick. SIX STREAMS ATTESTED all identical=true: GK gk-continuous-live (1800, 6dbb4341…), gk-shot-fixture-live (600, 4a1aa34d…); rules rules-corner-live (400, 0306d528…), rules-full-match-live (800, 2d909500…), designation-fullmatch-live (800, 4aa1cee9…), designation-throwin-live (1800, 6e78e38e…). Disclosed exclusions (budget/redundant; criteria not verdict-carried by attested streams owned by the still-accepted baseline records with explicit attribution): designation-arc-live, rules-throwin-baseline, rules-goalkick-postgoal-baseline, rules-throw-in-live, rules-goal-kick-live, rules-corner-goalkick-neighbour. UPDATED TABLES: goalkeepers 9 PASS / 0 FAIL / 2 NOT_EVALUATED / 1 BLOCKED_MISSING_REFERENCE / 1 NEEDS_PERCEPTUAL_REVIEW — the ONLY change vs the GK-SUITE-CORE-OWNED-STATE baseline (5cd1c808…, 8/3/1/1) is COMMON-DETERMINISTIC NOT_EVALUATED→PASS; rules the 25 §15 MATCH-* criteria byte-unchanged 23/2/0/0 with COMMON-DETERMINISTIC added as a determinism attestation row = PASS (26-row table 24/2/0/0; attested_per_criterion_regressions empty; suite-rules-v1 has common_criterion_ids: [] so the row is additive, not a §15 upgrade). VERDICT DERIVATION: PASS strictly from byte-identity — the critic proved the comparison real with a cpuAntiHuddle:false divergence probe and its own element-wise per-tick verifier (gk-shot-fixture-live 600/600 identical, rules-corner-live 400/400 identical). Evidence: record_sha256 abaf6ccdc8a07643289035e8dbfcc639a525a66955b4738f3735d24fa350a27c (no wall-clock field — keys verified; candidate_commit = HEAD 86a3a53; two consecutive ordinary-mode runs byte-identical to each other AND to the durable record, 7e995f97…; ordinary runs leave docs/ byte-identical — WIP gate verified by full-tree SHA comparison); audit PASS BOOKKEEPING. Tests: binding 7/7 (incl. the live discriminating determinism check) + rules gate 173/173 + GK families 88/88 + registry/provenance/hygiene 200/200 + foundation 28/28 + determinism/stateHash pins 59/59 + restart/match determinism suites 162/162; typecheck 0. Critic ACCEPT first pass (ran the producer twice itself; wrote its own verifier; programmatically verified the deltas); integration ACCEPT first pass (batteries re-run green with counts matching exactly; invariant gate semantics matched to compare-foundation.ts + the spec; silent-consumer hunt clean). claims_not_made: no suite-level PASS claim (both suites retain BLOCKED/NEEDS_PERCEPTUAL_REVIEW/NOT_EVALUATED rows; the accurate phrasing) / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented envelope / no criterion upgraded beyond the executed two-run attestation (COMMON-DETERMINISTIC PASS only on byte-identity, never forced) / zero gameplay change / no accepted record mutation.

## Iteration 184 — 2026-09-07

- objective_id: HUMAN-RESTART-CONTROL
- builder: builder-gameplay / deepseek-v4-flash (reroute)
- critic: critic / glm5.3-flash — RETRY→ACCEPT (the mechanism PASSed; the frames were visually void — 5 byte-identical PNGs with the event below the frame; fixed per 5 required fixes incl. the hardened PNG-byte guard)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass on the repaired candidate)
- result: accepted
- commits: 5b15992 candidate(HUMAN-RESTART-CONTROL)
- notes: Horizon v32 1/4. Human-directed restart destination control (DYNAMIC_VISUAL, audited at the strictest named class). INVESTIGATION: the core executes every restart and AUTO-SERVES at countdown zero toward the awarding team's nearest receiver (applyThrowIn simulation.ts:688-750; countdown loop :1688-1699 unconditional) — even a human pass at the window is overridden. REALIZATION (explicit `realization` object in the record): receiver-steering DESTINATION control through the movement-direction channel — when the human's team won the restart AND the window is active AND the human controls an awarding-team body (isHumanDirectedRestartActive: pure read-only predicates over committed WorldState), the human's directional input enters ONLY through sim.applyInputs, steers that body during the window, and the core's OWN nearest-receiver serve re-targets toward the human-chosen position (guard: (20,30) → (20.494974746830586, 29.505025253169414), directionChanged, gate 19/24 ticks). NOT a pass-button ball-server: the literal human ball-server is DEFERRED behind a disclosed core change (countdown-zero auto-serve override is core-owned; a pass needs ball contact, contact-system.ts:470-545, impossible during an out-of-play window) — the objective's no-core-change constraint forbids it; the critic verified the analysis from core source. CPU FALLBACK: no input → the core auto-serves exactly as today (the integration reviewer's stash probe re-derived the CPU-fallback serve byte-exactly: state_hash_of_hashes 79ef0552…; identical serve target + throw direction in a no-frame headless variant; the only chain delta vs zero-frame runs is pre-existing scheduler diagnostics). CRITIC RETRY: the first capture was visually void (5 byte-identical PNGs, the event 200-440 px below the frame — the browser test's distinctness guard checked world-state hashes, not PNG bytes) — fixed per the 5 required fixes: evidence camera re-placed ~18.8 m from the spot (~28 px/m; 5 DISTINCT PNGs 3aeff545/16091d63/c09af927/2bca4bb1/2869ccb5 with the steered receiver ring (259,217)→(275,210) and ball flight human-serve (559.9,319.7) vs cpu (552.1,316.7) verified legible by the critic's pixel forensics), the frame guard hardened to PNG-byte distinctness (≥4 distinct SHA-256 over captured bytes — a visually void capture can never pass again), sequence.json regenerated with per-frame tick + pngSha256 byte-bound to the on-disk PNGs, the reporting fixes (the unimplemented "input inert" docstring promise replaced with a pointer at the real gate-predicate guard; the restart battery named as 10 files 142/142 with the historical 121/121 label disclosed), and the realization disclosure added to the record. record_sha256 legitimately changed 5bba8a7c… → 42bb7ef0d5f916b0cc456e2f01a46fd8ec2c3c88b67eab7a2ff7b0b23e4f6bb5 (fix 5 added the required realization disclosure — intentional content, disclosed; no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical; docs/ byte-identical on ordinary runs). Zero tracked-file modification (8 additive untracked paths; git diff HEAD EMPTY; the new adapter module imported by nothing in the shipped app). Tests: integration 10/10 + restart battery 142/142 (97 unit + 45 integration across 10 files) + accepted-pin adapter guards 115/115 (integration re-ran 300 tests across 19 files) + browser test with the hardened guard; typecheck 0; audit PASS DYNAMIC_VISUAL. Critic (glm5.3-flash) RETRY→ACCEPT (semantic_audit VALID after pixel forensics); integration (glm5.3-flash) ACCEPT first pass on the repaired candidate. claims_not_made: no PES fidelity / no FOUNDATION_LAB_PASS / no suite-level PASS / no claim that the human literally serves every restart type (corner's cross target is a fixed point; goal-kick uses the same receiver-steering semantics but is not separately evidenced) / no organic-restart claim (fixture-driven via restore, disclosed) / no football-rule change / no core change.

## Iteration 185 — 2026-09-07

- objective_id: HUMAN-RESTART-RULES-CONFORMANCE
- builder: builder-structured / deepseek-v4-flash
- critic: critic / glm5.3-flash — RETRY→ACCEPT (the freeze-exemption gate leaked — fired on body-id match without requiring humanDirected=true; fixed with the window-scoped humanWindowTaken marker)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass on the repaired candidate)
- result: accepted
- commits: 2a6b92c candidate(HUMAN-RESTART-RULES-CONFORMANCE)
- notes: Horizon v32 2/4. The human-taken restart conforms through the rules suite — the human restart satisfies the SAME rules the CPU restart does. OUTCOMES: on the human-taken NATURAL boundary stream (genuine ball-out-of-play, team-b wins) all 7 applicable criteria PASS identically to the CPU restart (PLACEMENT/SERVE/THROW-IN-TIMER-FREEZE/AWARD/FREEZE-UNTIL-FIRST-TOUCH/NEAREST-ONLY; REARM honestly NOT_EVALUATED); on the human-taken driven window SERVE/TIMER-FREEZE/FREEZE-UNTIL-FIRST-TOUCH PASS with PLACEMENT/AWARD/NEAREST-ONLY/REARM honestly NOT_EVALUATED; the CPU fallback yields the SAME criteria verdicts with DIFFERENT serve targets proving the human directed the destination (driven: human (20.495, 29.505) vs CPU (20, 30); natural: human (26.224, 27.883) vs CPU (22.106, 32)). SERVE oracle semantics UNTOUCHED (attests serve legality, not receiver choice). REALIZATION: receiver-steering destination control, NOT a pass-button ball-server (disclosed). THE GATED EXTENSION (after the critic RETRY — the first candidate's freeze exemption leaked, firing on body-id match without requiring humanDirected=true; verdict-inert only by fixture luck): restart-designation payloads now carry (humanDirected, humanTeamId, humanControlledPlayerId, humanWindowTaken) ONLY on human-taken windows — humanWindowTaken is a WINDOW-SCOPED marker computed post-loop over maximal runs of consecutive ballUntouched ticks (marked iff humanDirectedByTick is true on at least one tick); the freeze exemption fires ONLY on humanWindowTaken===true && humanControlledPlayerId!==null && body-id match — a CPU-fallback stream carries ZERO human fields BY GATE (all three tick counts 0 on both CPU-fallback streams, guard-attested) so its freeze behavior is byte-identical to pre-change by gate, not fixture luck. The driven stream's FREEZE PASS is load-bearing on the exemption and now DOCUMENTED: the window never closes (ticks 0-24); the human directed ticks 0-18; the post-window coast (ticks 21-23, drift 0.759-0.872 m vs 0.75 tolerance) is legitimately exempt as the human-directed window's sanctioned steering momentum — the pre-change counterfactual FAILs exactly there; the rationale disclosed in the record + RESULT.md (explicitly NOT a per-tick humanDirected leak). Binding tests 10/10 incl. the FAIL-direction checks: non-human body drift in a marked window FAILs; human-slot body drift on a CPU-fallback stream FAILs; id-without-marker does NOT exempt. The natural stream conservatively carries no marker (the designation's untouched run is the post-serve flight while the human steers during the countdown; the human body checked and held at 0.000; directedness evidenced by the serve-target guard) — disclosed in marker_presence_detail. Evidence: trajectory 8 runs; record_sha256 44c0d928a8576860a6fd36acf87f785223ef5ec837503fcdc27a2356a66bf237 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical; docs/ byte-identical on ordinary runs); audit PASS MULTI_TICK. Tests: binding 10/10 + rules gate 156 + stateHash pins 69 + accepted prior streams' bindings verdict-unchanged + restart battery + human-restart-control integration 58 + foundation/provenance 71 + architecture 27; typecheck 0. Critic (glm5.3-flash) RETRY→ACCEPT (caught the leak synthetically AND live; verified the fix mechanically with zero leak rows + the old-leak pin); integration (glm5.3-flash) ACCEPT first pass — 268/268 across 16 files, the strongest neighbor check: all 5 accepted gated streams re-run BYTE-IDENTICAL through the post-change runner (hash chains match the RESTART-DESIGNATION-FACTS + CORNER-DRIVEN pins), the exemption's four edge probes (marker-true/null-id FAIL, marker-false/body-id FAIL, other-body FAIL, same-body exempt PASS), leakRows=0. claims_not_made: receiver-steering realization, NOT a pass-button ball-server / no suite-level PASS claim / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented constant / blocked references stay blocked / no criterion upgraded beyond the executed evaluator / no core or spec change.

## Iteration 186 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: fec4a35 candidate(EVAL-HYGIENE-CONSOLIDATION)
- notes: Horizon v32 3/4. The four reviewer non-binding cleanups consolidated with guards (BOOKKEEPING; zero verdict changes — proven directly, not just claimed). ITEM 1: the dead `chasers` Set removed from checkRestartNearestOnly (genuinely unread; the clump-count falsifier documented; rules gate 173/173 + the accepted-designation-stream pin replay record_sha256 271b1526… byte-identical prove verdict-inertness). ITEM 2: the serializeRestartFacts designation comment tightened (the adapter-phase / POST-STEP-ball-reference pairing and its conservative direction stated; comment-truth verified against the step/snapshot ordering; comment-only). ITEM 3: the _keeperPressExclusions counter interaction DOCUMENTED (post-loop assignChaseRoles → designatePresser inflates the counter; never combine a gated designation run with a GK counter read in one process — reset between them; documentation-only — the counter stays a diagnostics value, never gameplay). ITEM 4: the HUD ortho camera resize re-anchoring (renderer.ts — a container ResizeObserver re-anchors the HUD ortho camera + top-left sprite + renderer canvas + main camera aspect; getHudCamera() accessor; presentation-only/draw-only; the observer lifecycle leak-free — reset/dispose/initScene disconnect-and-null, no double registration, frustum clamped ≥1px) with the browser guard 3/3 (frustum re-anchor 800×600→1200×800, canvas follows container, HUD top-left, null camera when disabled). ZERO VERDICT CHANGE proven directly: the critic re-ran the gated designation stream (record 271b1526… byte-identical, all 25 criterion verdicts identical) and the integration re-ran the 173/173 gate + all accepted-stream bindings; the render byte-identity proven by the pinned keeper-marker PRE_CHANGE_BASELINE_SHA 05e40d0117…68bc staying EXACT + the archetype identical-recapture + all screenshot families (the HUD-off default path byte-neutral; the ResizeObserver's initial observe a no-op at default sizes). Evidence: record_sha256 30d933e2b3245c0c61cbde132aed75bfc4005417cb79f2db72611044466d7675 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical; ordinary runs leave docs/ byte-identical — full-tree sha256 sweep); audit PASS BOOKKEEPING. Tests: rules gate 173/173 + stateHash pins + GK/registry battery 309/309 (integration) + anti-huddle integration 43/43 + HUD guard 3/3 + render-sensitive batteries 63/63 across 19 files + cpu-adapter guards 45/45 + architecture 27/27; typecheck 0. Critic ACCEPT first pass (every claim independently reproduced — comment-truth verified against code, the counter increment path verified, the observer lifecycle read in full); integration ACCEPT first pass (every battery re-executed independently; rules-oracle chunked 18+19+21+17 covering all 75 tests — full single-invocation runs exceed the 300s command cap on the 2-CPU host, a harness quirk disclosed, not a candidate defect). claims_not_made: zero verdict changes (every oracle check's semantics byte-identical post-cleanup) / no suite-level PASS claim / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity / no core change (the renderer HUD change is presentation-layer) / no accepted evidence record mutation.

## Iteration 187 — 2026-09-07

- objective_id: FULLTIME-FLOW-CLOSURE
- builder: builder-gameplay / deepseek-v4-flash (reroute)
- critic: critic / glm5.3-flash — RETRY→ACCEPT (the menu-return frame was a rebuilt lookalike presented as the shipped menu; replaced with a genuine capture of the real shipped #setup-menu-card)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass on the repaired candidate)
- result: accepted
- commits: 3f42a50 candidate(FULLTIME-FLOW-CLOSURE)
- notes: Horizon v32 4/4 (horizon COMPLETE). The fulltime dead end is closed — the playable match loop is complete (DYNAMIC_VISUAL, audited at the strictest named class). INVESTIGATION (reproduced from HEAD by the critic): the real-time gameLoop previously kept running PAST fulltime — sim.step() still fired every frame, players/ball kept moving in a non-gaming post-fulltime state, the scoreboard kept counting, the transient overlay faded after ~1 s, no rematch affordance. THE FIX (presentation layer only; git diff src/simulation/ src/contracts/ EMPTY): at the core-owned fulltime terminal state the loop now FREEZES (the terminal frame paints, then return before the next rAF — scoped exactly to fulltime; no pre-fulltime behavior change; all accepted pins/hashes unchanged); an affordance: renderer.ts opt-in `showFulltimeFlowHud` (default FALSE — byte-identical when off, the keeper-marker baseline SHA 05e40d0117…68bc EXACT) draws `[R] REMATCH  [M] MENU`; src/apps/browser/fulltime-flow.ts (new, zero imports) provides the clickable REMATCH / BACK TO MENU panel + M/R keyboard dispatcher GATED so in-play controls (WASD/Shift/K/J/L/Tab/E+J/Q+J/U/I — none collide) are NEVER intercepted (returns false until markFulltime; hide() clears on startMatch/showSetupMenu); main.ts: onRematch → startMatch(lastMatchConfig) (the composition-root match-start path), onBackToMenu → stopMatch() + showSetupMenu(); the panel is a runtime overlay — ladder-menu-parity stays green. FRAMES (4 byte-distinct, each unique tree-wide): fulltime-terminal (5fb989c0…) → fulltime-affordance (226e987f…) → menu-return (ba9d4473…, the REAL shipped #setup-menu-card captured via the new dev-server Playwright script driving the REAL page through #start-button → real match → the shipped #back-to-menu button — the same handler the M dispatcher calls, disclosed; the Difficulty row + the full 9-option ladder + the 10-row controls legend cross-checked against DEFAULT_KEYBOARD_CONFIG) → new-match-start (ae3ccf3b…, a fresh kickoff). CRITIC RETRY resolved: the first menu-return frame was a rebuilt lookalike (buildSetupMenuCard — no DIFFICULTY row, single mode option) presented as the shipped menu (semantic_audit INVALID); fixed per the 4 required fixes (the real capture, the lookalike removed — grep zero matches, the metadata drift corrected — fulltime_tick 660 == fulltime_frame_tick 660 and the correspondence direction corrected to browser-EARLIER, the stale hash line refreshed); semantic_audit VALID after the fix; record_sha256 legitimately changed a4809f31… → 8f58216742d1cb6806e729f339730a1668191256d9c46e6f2fba131fdf459cb2 (corrected state facts/disclosures — disclosed in RESULT.md; no wall-clock field, no PNG bytes embedded; reproduced byte-exact by critic AND integration across full ordinary pipelines). Evidence: audit PASS at --class DYNAMIC_VISUAL (semantic visual sequence + screenshot SHA uniqueness PASS). Tests: browser test 7/7 + ladder-menu-parity 9/9 + hud-ortho-resize 3/3 + keeper-visual-marker 4/4 + browser-full-match-flow-evidence 3/3 + render-sensitive 24/24 + human-mode suites all green (23/23 + 26/26 + 2/2 + 17/17 + 13/13 — in-play keys unaffected) + node gates 8/8 (capture-hygiene's writer-gate addition a single line enrolling the new suite — enforcement, not weakening) + typecheck 0. Integration (glm5.3-flash) ACCEPT first pass on the repaired candidate — every suite re-run independently, the loop-freeze scoping verified (after the terminal frame's render, before the next rAF), fulltime-flow.ts consumed only by main.ts + its test, both paths through the composition root, a 990-file docs/ SHA sweep byte-identical across its reproduction cycles. Known gaps disclosed: the driven 240-tick-half fixture (not organic 90-minute); the DOM-capture menu frame; the 1-tick browser↔headless fulltime offset (660 vs 661 — browser EARLIER); no per-tick hash-identity claim; the composition-root end-to-end verified via module behavior + the wiring guard + the correspondence test rather than a full real-time run to fulltime (URL scenarios use 5400-tick halves — can't reach fulltime in a test budget). claims_not_made: no PES fidelity / no FOUNDATION_LAB_PASS / no suite-level PASS / no organic 90-minute claim / no per-tick browser-headless hash-identity claim / no claim the rematch is a core change / no invented reference tolerance / no football-outcome change beyond the presentation surface.

## Iteration 188 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: b7f56e0 candidate(FOULS-SPEC-DRAFT)
- notes: Horizon v33 1/4. The dedicated fouls/cards spec drafted (spec-only; git diff src/ eval/ gauntlet/ EMPTY — zero implementation, verified by builder, critic, AND integration). SPEC: specs/FOULS_CARDS_SPEC.md (15 sections; the naming decision disclosed — FOULS_CARDS_SPEC because MATCH_RULES_SPEC §16 groups fouls/cards/free-kicks) following the RULES-SPEC-DRAFT pattern: the foul definition GROUNDED in the accepted duel/tackle machinery (a defensive-tackle active-window player-player-contact with contactType ∈ {standing-tackle, slide-tackle}, tacklePhase === "active", and duelWon === false (ballReachable === false) — the man-not-ball contact the existing tackle system already produces; duelWon = ballReachable at tackle-system.ts:505; the critic verified the fields match the source EXACTLY and found 10 real man-not-ball slide-tackle events in the accepted duels-suite evidence, with the clean complement's 65 duelWon:true events confirming the exclusion and PHY-SHLD-001-CONT the shoulder-contact reference); the NOT-a-foul complement; carrier identity (§5.3 honestly discloses the tackle event does not serialize carrier distance); advantage/cards NAMED-NOT-IMPLEMENTED; the free-kick consequence DEFERRED and forward-referencing the ACCEPTED restart machinery; fouls-v1 VERSIONED_PROVISIONAL (prose-declared — no eval/contracts/fouls-config.ts, disclosed); FIVE criteria NAMED-but-NOT-REGISTERED (FOUL-DETECT, FOUL-CLEAN-TACKLE, CARD-ISSUED, ADVANTAGE-PLAYED, FREE-KICK-AWARD) with the §10 guard; offside/penalties explicitly regulation-only with no existence claim; 7 BLOCKED_MISSING_REFERENCE disclosed; all 10 cross-referenced spec sections verified. BINDING: fouls-spec-binding.test.ts 42/42 — every quoted §9.2 constant pinned to its machine source (structurally live: the producer recomputes constant_drift=[] live; the critic hand-verified the 10 unpinned values) and the named-criteria-unregistered assertions as REAL registry lookups (independently confirmed by grep — zero occurrences anywhere). Evidence: record_sha256 e98a1efe08f1c8ec68c5b957a801e9035c70a132e3f5d354e3dd30b8767b0716 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical; docs/ byte-identical on ordinary runs); audit PASS HEADLESS. Tests: binding 42 + neighbors 376/376 (integration) + 27/27 architecture + 8/8 hygiene + 118/118 determinism/replay (critic re-ran ~683 targeted tests incl. core units 246/246); typecheck 0. Disclosed gap: a full `pnpm run test` exceeds the 5-minute tool window (mitigated by the empty tracked diff + ~683 reproduced passing tests — honestly disclosed, reproduced, accepted as negligible-residual). Critic ACCEPT first pass; integration ACCEPT first pass (the silent-consumer hunt found no doc-lint/spec-check pipeline the spec could break). claims_not_made: no implementation claim / no criterion registered / no fouls behavior exists / no PES fidelity or invented constant (fouls-v1 is VERSIONED_PROVISIONAL) / offside/penalties stay regulation-only / no PROMOTION / no FOUNDATION_LAB_PASS / blocked references stay BLOCKED_MISSING_REFERENCE.

## Iteration 189 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 3ef0edd candidate(HUMAN-KEEPER-CONTROL)
- notes: Horizon v33 2/4. The human can control the designated keeper in human-vs-CPU 5v5 (DYNAMIC_VISUAL). INVESTIGATION (reproduced by the critic from source): the core switch contract does NOT exclude the keeper — computeExplicitSwitchTarget (input-system.ts:486-511) cycles all teammates and the core switch handler cycles to it, so the keeper was Tab-reachable pre-change; the REAL gap was that the small-sided keeper role (gkBehavior) was OFF for human-vs-CPU modes (main.ts gated keeperPids/displaySnapshot/GK_5V5 on IS_AI_MATCH_5V5 only). THE FIX (zero src/simulation/ + src/contracts/ change): (1) main.ts: IS_KEEPER_ROLE_LIVE = IS_AI_MATCH_5V5 || IS_HUMAN_VS_CPU_5V5 applied to the three gates — the readable keeper kit-marker and role work in human-vs-CPU 5v5; CPU-vs-CPU unchanged; non-5v5 modes take the identical pre-role path. (2) cpu-adapter.ts: CpuObservation.humanDirectedKeeperMove — computeKeeperFrame yields the arc-hold positioning to the human's directional input (scaled by the in-arc keeper_reposition_speed cap) while the save/claim reaction (FIRST_TOUCH_BIT) + distribution (PASS_BIT) stay on the shared production rules and never read humanMove; the absent-field else branch is the exact original arc-hold code; input enters only through tick-indexed InputFrame. (3) src/adapters/input-browser/human-keeper-control.ts (new read-only gates). (4) The driven fixture 5v5-human-keeper-shot-fixture.v1.json + eval/runners/human-keeper-control.ts (humanMode directed vs cpu) + the capture script. The keeper designation via designateKeeperFromLayout UNCHANGED — the human directs the already-designated body, never becomes the designation. FRAMES (4 event-centered, byte-distinct): human-keepers-directed (@331) → shot-on-target (@343, the shooting body's canonical CPU SHOT press) → save-contact (@364, the human-directed keeper answers 1.0787 m off the ball within save_claim_reach_radius 1.2) → cpu-keeper-control (@364, no input, auto-save 1.1079 m). HONEST DISCLOSURES (verified by the critic): fixture-driven (the shot is the shooting body's canonical CPU SHOT press, not a scripted ball); the standard-wiring nuance TRUE BY COMPUTATION — designateKeeperFromLayout on 5v5-human-vs-cpu.v1.json yields team-a → player-1, exactly the human's default slot-1, so in the standard browser wiring the human's team keeper IS the human's default controlled body and no auto-save is claimed there (the core routes the human slot's frame to the shared body); the reliable human-directed save is on the driven fixture. Evidence: record_sha256 511d53df386ee634bf545abe19addfad10c419964055c2d579709ce3b2baabe0 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical; docs/ byte-identical on ordinary runs); audit PASS DYNAMIC_VISUAL. Tests: unit 5/5 + integration 6/6 (incl. gkBehavior:false stash identity) + node neighbors (integration re-ran 529 node tests + switch/indicator 15/15 + GK-5V5 15/15 + human-mode suites all green + the keeper-marker baseline SHA exact) + browser 1/1; typecheck 0. Critic ACCEPT first pass (two non-blocking reporting notes: the human_controllers_keeper guard computed tautologically; the "CPU keeper adapter does not run" phrasing mechanically loose though the disclosed consequence accurate). Integration ACCEPT first pass — an independent CPU-vs-CPU byte-identity probe: live tree vs accepted pre-candidate chains (gkBehavior ON shot 6b7fa24c…, continuous 660c075a…, OFF 6091de51…/6817faad…) all four BYTE-IDENTICAL; the shipped app does not import human-keeper-control.ts (it uses the gate only through IS_KEEPER_ROLE_LIVE); human-mode suites all green. claims_not_made: no PES fidelity / no FOUNDATION_LAB_PASS / no suite-level PASS / no invented envelope / no claim the human becomes the keeper designation / no claim of an auto-save in the standard browser wiring (disclosed; the reliable human-directed save is on the driven fixture) / no core change.

## Iteration 190 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: b55fa74 candidate(HUMAN-BALL-SERVER-DECISION)
- notes: Horizon v33 3/4. The decision record for the deferred literal pass-button restart server (BOOKKEEPING; zero source change — verified by builder, critic, AND integration; the decision is recorded, not executed). ANALYSIS (verified at source level by the critic): the countdown-zero branches (simulation.ts:1674-1681 corner, :1689-1700 throw-in, :1702-1712 goal-kick) fire UNCONDITIONALLY with no input path; a literal human ball-server needs a PASS-GATED SERVING PATH (at countdown zero, if the designated taker is human-controlled, keep the restart phase open and wait for a human PASS_BIT InputFrame within a bounded window; on the pass, derive the serve direction from input and execute; else the CPU auto-serve fires); it cannot reuse the contact-system pass path (a pass requires the player within passRadius 1.2 m — foundation.ts:159,182 — of a contactable ball, which does not hold during an out-of-play window; contact-system.ts:429 gates the candidate scan on dist <= effectiveRadius). DETERMINISM/PIN IMPLICATIONS: the auto-serve is countdown-fixed today (accepted pins computed over that fixed serve tick — spot-checked against the accepted RESTART-RULES-CONFORMANCE stream); a human-gated serve introduces a VARIABLE WAIT TICK (deterministic under a fixed input program but input-timing-dependent); the two-run DETERMINISTIC attestation runs CPU streams so unaffected as long as the CPU fallback stays byte-identical; a new human-served stream needs its own attestation; the restart-window machinery (humanWindowTaken, freeze exemption, re-arm, first-touch close) is affected because the window closes on the pass. CONFORMANCE IMPLICATIONS: THROW-IN-SERVE (and analogues), TIMER-FREEZE criteria, FREEZE-UNTIL-FIRST-TOUCH/KICKOFF-FIRST-TOUCH (window-close semantics), MATCH-TIMER-FREEZE would need re-evaluation on human-served streams. DECISION: DEFER (not_a_hedge recorded) with five concrete reasons — (1) the delivered receiver-steering realization already covers the meaningful human influence on restart destination/direction for the primary restart types; (2) the literal server requires a core change with real determinism/pin costs; (3) no registered conformance path exists for a human-served stream; (4) the next horizon item has higher value; (5) the receiver-steering control is not left incomplete — it is the engine-supported realization. Four alternatives enumerated with covers/does-not-cover/cost, and the HUMAN-BALL-SERVER-LITERAL future-objective outline is ACTIONABLE (it names the conformance path: new oracles for human-chosen serve direction, wait-phase timer freeze, window-close/first-touch semantics; re-evaluation of SERVE/TIMER-FREEZE/FIRST-TOUCH on human-served streams; CPU fallback byte-identity; its own two-run attestation). Evidence: record_sha256 5b6e391a3d8da585b4e48bfae703ec9d57b78b1ecc1e4a10cbcfeeef5891b489 (no wall-clock field; reproduced byte-exact by critic AND integration; two consecutive ordinary runs byte-identical, scratch byte-identical to the durable record proving the gate changes location only; ordinary runs leave docs/ byte-identical — 1008-file snapshot); audit PASS BOOKKEEPING. Tests: binding 7/7 (real strict assertions — a changed or hedged verdict fails) + rules gate 180/180 (11 files) + restart battery + human-restart neighbors 188/188 (13 files; the known vitest worker-RPC artifact) + architecture 27/27; typecheck 0. Critic ACCEPT first pass (the analysis truth verified at source; the decision clarity verified); integration ACCEPT first pass (every battery re-run; the scope confirmed as exactly the decision-record item with no creep — one 6-file architecture run initially showed 2 toolchain-pnpm fails from the reviewer's own parallel battery contention; solo re-runs 27/27). claims_not_made: no implementation (the decision is recorded, not executed) / no suite-level PASS claim / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or measured envelope / no invented reference envelope / zero gameplay/source change.

## Iteration 191 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 9a7cb7f candidate(RELEASE-0.9.7-CONSOLIDATION) + 2ca140c prompt-gate needle advance (orchestrator-routed maintenance, the RELEASE-0.9.5 precedent) + a sanitizer fix in persist-acceptance.ts (dotted objective IDs) — all disclosed below
- notes: Horizon v33 4/4 (horizon COMPLETE). The v29-v33 gains consolidated into gauntlet/RELEASE-0.9.7.md (the established RELEASE-0.9.3/0.9.4/0.9.5/0.9.6 pattern): (1) PLAYABLE — the complete small-sided loop: setup menu → 1v1-5v5 human-vs-CPU + CPU-vs-CPU ladder → human controls incl. the designated keeper (HUMAN-KEEPER-CONTROL) + human-directed restart destination (HUMAN-RESTART-CONTROL) → duels → GK behavior → visible full-match lifecycle (halftime/fulltime) → fulltime rematch/menu (FULLTIME-FLOW-CLOSURE); (2) EXECUTABLE/ATTESTED — rules suite 23/2/0/0 (+24/2 with COMMON-DETERMINISTIC; RULES-SUITE-STATE-RERUN 36fc77e5…); goalkeepers 9/0/2/1/1 (GK-SUITE-CORE-OWNED-STATE 5cd1c808…); COMMON-DETERMINISTIC two-run attested (SUITE-DETERMINISTIC-TWO-RUN abaf6ccd…); 8 protected oracles; the gated designation-facts serialization (271b1526…); (3) SPEC'D — FOULS_CARDS_SPEC (fouls-v1, spec-only; e98a1efe…); (4) DEFERRED — the ball-server DEFER + the HUMAN-BALL-SERVER-LITERAL actionable outline (5b6e391a…); the blocked references; GK beyond small-sided; regulation; full-match ecology; (5) HONEST LIMITATIONS. Every claim cites its accepted record (record_sha256 pinned; the producer throws on mutation). VERSION.json 0.9.6 → 0.9.7 (previous_system_version → 0.9.6; baseline_commit → f661b38; includes unchanged). ORCHESTRATOR-ROUTED MAINTENANCE (both disclosed): (a) the builder correctly disclosed that the VERSION bump breaks the prompt-gate semver needle — the orchestrator advanced the needles (0.9.6→0.9.7, 0.9.5→0.9.6) as atomic commit 2ca140c AFTER the candidate 9a7cb7f (the RELEASE-0.9.5 precedent) and verified `gauntlet:eval` → 39 scenarios + 33 prompt checks PASS; (b) the FIRST-EVER dotted objective ID exposed a persist-acceptance sanitizer gap (dots → underscores moved the manifest off the canonical durability-gate path — caught by the integration reviewer across all 149 manifests) — the orchestrator fixed the sanitizer to preserve dots (atomic commit, below) and re-persisted so the manifest lands at docs/evidence/RELEASE-0.9.7-CONSOLIDATION/manifest.json. ORCHESTRATOR PROCESS CORRECTION (disclosed): the acceptance record was pre-persisted before the critic/integration reviews ran (the verdicts in the first draft were predicted, not real) — caught by the critic's own review (the pre-persist flag) and repaired before the acceptance commit: both pre-persist drafts deleted, the real reviews run (critic ACCEPT + integration ACCEPT), and the record re-persisted with real provenance (gauntlet_version corrected to 0.9.7 to match the bumped VERSION.json). Evidence: record_sha256 94ac0110f767c5ce5a5b9507c03846fe9e8c597f3933ab2f76bbf84f9568ca6c (no wall-clock field; two consecutive ordinary-mode runs byte-identical, file sha b4628e6c…; ordinary runs leave docs/ byte-identical); audit PASS BOOKKEEPING. Tests: binding 10/10 + rules gate + new binding 190/190 (12 files) + registry/provenance/hygiene 70/70 + architecture 27/27 + gauntlet:eval 39+33 PASS (with the advanced needle) + gauntlet:eval:state 13/13; typecheck 0. Critic (glm5.3-flash) ACCEPT first pass — all pinned SHAs recomputed (36fc77e5, abaf6ccd, 5cd1c808, 7503f9fe, 271b1526, e98a1efe, 5b6e391a), the verdict tables recomputed from the records themselves, the version diff verified exact, the needle advance verified with its own gauntlet:eval run; integration (glm5.3-flash) ACCEPT first pass — 389/389 neighbor tests + 256/256 determinism/replay + 30/30 architecture + gauntlet:eval:state 13/13 + the 149-manifest dir/objective_id scan that caught the sanitizer gap. claims_not_made: no suite-level PASS claim (rules 2 BLOCKED; goalkeepers 1 BLOCKED + 1 NEEDS_PERCEPTUAL_REVIEW + 2 NOT_EVALUATED) / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity / no invented envelope / no criterion upgraded beyond the executed evaluator / every PASS cites an accepted record / zero gameplay/source change / no accepted record mutated.


## Iteration 192 — 2026-09-07

- objective_id: FOUL-DETECTION-MACHINERY
- builder: builder-gameplay / deepseek-v4-flash (reroute)
- critic: critic / glm5.3-flash — ACCEPT (first pass)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 936a297 candidate(FOUL-DETECTION-MACHINERY)
- notes: Horizon v34 1/4. FOULS_CARDS_SPEC §5 realized as observation-level detection (MULTI_TICK) with ZERO gameplay change — git diff src/ src/contracts/ EMPTY (verified by builder, critic, AND integration); the gk-role/designation-facts precedent. THE CHANGE: eval/runners/headless-match.ts +31 (default-false `detectFouls` option; post-loop injection after the serializeRestartFacts block, before match stats — inputs/steps/state hashes already committed, provably unaffected); eval/runners/foul-detection.ts (pure producer detectFoulEvents/countFoulEvents/FOUL_CONTACT_TYPES/FoulEventPayload); scripts/capture-foul-detection-machinery.ts; tests/unit/eval/foul-detection-machinery-binding.test.ts (12 tests); package.json +1 script. PREDICATE per FOULS_CARDS_SPEC §5.1 exactly (contactType ∈ {standing-tackle, slide-tackle}, tacklePhase === "active", duelWon === false; duelWon = ballReachable at tackle-system.ts:505); fields mirror the committed contact payload (§4.2) plus sourceEventId; §5.2 complement tested (clean tackle + symmetric shoulder excluded). EVIDENCE: driven-foul-live 1 foul (tick 51, standing-tackle, payload valid; 300 contacts examined), organic-foul-live 1 foul (tick 536, slide-tackle; 73 contacts), organic-foul-stashed detectFouls:false → 0 fouls, state_hash_chain_identical true, byte-identical to the pre-change baseline (hashOfHashes fb5e9b02…). Evidence: record_sha256 69104ea10e14aa2607eefbd9e3206e17d025f4cea5206032867e4772bc89a272 (no wall-clock field; reproduced byte-exact independently by critic AND integration; ordinary runs leave docs/ byte-identical — hash-sweep verified). Audit PASS MULTI_TICK (20 checks: 17 PASS + 3 NOT_APPLICABLE for the headless class). Tests: binding 12/12; typecheck 0; integration headless-match/set-piece/scoring/lifecycle/replay 102 passed; neighbor eval bindings 85 passed; runner neighbors 40 passed (batteries chunked, disclosed). Critic ACCEPT first pass (reproduced the predicate against tackle-system.ts:541-590, the injection at headless-match.ts:1470-1472, and the record byte-exact; non-blocking: the audit has 20 entries vs the builder's reported 17 PASS — cosmetic; bare tsc shows 49 pre-existing errors outside candidate files; the organic count guard is >=1 — defensive looseness). Integration ACCEPT first pass (32 files / 381 neighbor tests 0 failures with the gate off; zero pre-existing consumers of the new exports — strictly additive; docs/ byte-identical on an ordinary run; package.json script unique; no builder commits; non-blocking: core-ts-isolation's tsc probe flakes under parallel vitest load, solo green; durability gate horizon check fails at post-replan HEAD as expected pre-bookkeeping). Provider failures: three bounded 401 NaN-gateway auth blips (transient per the harness signature; retries preserved the same logical inference via session resume — builder ×3, integration ×1; no state advancement from failures). claims_not_made: no suite verdict or oracle registration for FOUL-DETECT / FOUL-CLEAN-TACKLE (named-but-unregistered per §10) / no card, advantage, or free-kick implementation (spec-only) / no PROMOTION / no FOUNDATION_LAB_PASS / no milestone or regression PASS / no PES fidelity or invented envelope / no gameplay/source/contract/adapter/spec change.


## Iteration 193 — 2026-09-07

- objective_id: FOULS-SUITE-REGISTRATION
- builder: builder-structured / deepseek-v4-flash
- critic: critic / glm5.3-flash — ACCEPT (first pass)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: b7b7a1a candidate(FOULS-SUITE-REGISTRATION)
- notes: Horizon v34 2/4. The two FOULS_CARDS_SPEC §10 criteria the accepted FOUL-DETECTION-MACHINERY machinery makes answerable — FOUL-DETECT and FOUL-CLEAN-TACKLE — registered as executable protected oracles (MULTI_TICK; zero gameplay change — git diff src/ src/contracts/ EMPTY and the producer eval/runners/foul-detection.ts unmodified, verified by builder, critic, AND integration). REGISTRATION (RULES-SUITE-REGISTRATION pattern, field-for-field): FOULS_SUITE (suite-fouls-v1, common_criterion_ids [], impact_closure NONE); FOUL-DETECT + FOUL-CLEAN-TACKLE HARD_INVARIANT; invariants foul-detect-evidence/foul-clean-tackle-evidence → protected oracles foul-detect-oracle-v1/foul-clean-tackle-oracle-v1 (wire.ts + CRITERION_TO_ORACLE, both additive, zero existing rows changed); bindings BINDING_FOULS_DETECT_001/BINDING_FOULS_CLEAN_TACKLE_001; obs-fouls-v1; config-fouls-v1/expansion-fouls-v1; scenario scn-fouls-lifecycle-v1. Registry 58 bindings / 8 suites / 95 criteria / 38 invariants / 52 scenarios; content_hash fnv1a64-v1:8185c0af89cf8a39 (grew for the new suite; the provenance/registry gate is the GK-SPEC-SUITE-CONTRACTS format+non-placeholder+tamper-discriminant accommodation — no literal hash pin needed, verified live). CARD-ISSUED / ADVANTAGE-PLAYED / FREE-KICK-AWARD stay named-not-registered (grep-verified; binding tests enforce the absence both directions). EXECUTED VERDICTS over the accepted detection streams (state_hash_of_hashes character-identical to the accepted values: driven 8c227ab4…, organic fb5e9b02…): driven-foul-live FOUL-DETECT PASS + FOUL-CLEAN-TACKLE PASS; organic-foul-live PASS + PASS (critic independently reproduced both); organic-foul-stashed (gate off, 0 fouls) both honestly NOT_EVALUATED. 4 mutant canaries FAIL (non-spec contactType foul; duelWon-true-sourced foul; shoulder-sourced foul; false-positive guard). Evidence: record_sha256 5e538e5d183ee0c3c79e4981bb447a06c42d3b7be3a3ceb66f320dfb24611794 (no wall-clock field; reproduced byte-exact independently by critic AND integration; ordinary runs leave docs/ byte-identical; durable writes under WIP_SECTION=__EVIDENCE__:FOULS-SUITE-REGISTRATION); audit PASS MULTI_TICK (20 checks: 16 PASS + 4 NOT_APPLICABLE). Tests: fouls-suite-binding 18/18; eval-registry 48/48 (56→58 disclosed); fouls-spec-binding 42/42; FOUL-DETECTION-MACHINERY binding 12/12 green; RULES-SUITE-REGISTRATION 28/28; oracle-registry 19/19; typecheck 0; integration ran 608 node tests across all 20 registry-consumer files + 2 canary suites (rules/goalkeepers verdict tables + pinned record SHAs unperturbed; gauntlet:eval 39+33; eval:state 13/13). Disclosures: tests/unit/eval chunked (>300s, the established precedent); one pre-existing vitest worker-RPC artifact in a reviewer batch (7/7 green); the suite is NOT wired into the browser composition root (set_by disclosed); a binding-test header typo (cosmetic); the spec §10 heading wording ("named, NOT registered") now factually outdated for the two registered criteria — recorded as a disclosed open thread for the next natural fouls objective. claims_not_made: no card/advantage/free-kick machinery or verdict / no suite-level PASS (2 of 5 §10 criteria registered) / no PROMOTION / no FOUNDATION_LAB_PASS / no milestone or regression PASS / no PES fidelity or invented envelope / no gameplay/source change.


## Iteration 194 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 9a04158 candidate(HUMAN-BALL-SERVER-LITERAL)
- notes: Horizon v34 3/4. The scheduled-ready HUMAN-BALL-SERVER-DECISION outline (5b6e391a…) executed — the horizon's DELIBERATE core change (MULTI_TICK). src/simulation/loop/simulation.ts: the three countdown-zero restart branches (corner/throw-in/goal-kick) gate on a closure-held humanServeWaitTicks + isTakerHumanControlled; a human-controlled designated taker holds the restart phase open (matchTimer frozen — decrement only under playing), findHumanPassFrame consumes the tick's PASS_BIT, applyThrowInFromInput/applyGoalKickFromInput/applyCornerKickFromInput serve along the human's input direction (bodyHeading fallback); expiry/CPU-taker executes the exact pre-change statements. HUMAN_SERVE_WAIT_WINDOW_TICKS=90 declared match-rules-v1 VERSIONED_PROVISIONAL with a doc comment. src/contracts/scenario.ts +1 event kind restart-serve-wait (additive, only on human-serve streams — grep-verified). Eval: eval/oracles/human-serve.ts (3 protected oracles: direction/timer-freeze/window-close) + wire.ts + CRITERION_TO_ORACLE additive; headless-match.ts humanRestartControl.humanPassAtTick (PASS via tick-indexed InputFrame) + restart-serve-wait injection (default-off, hash-neutral); scenario 5v5-human-serve-throwin.v1.json; 3 new test files. GATE SEMANTICS (critic-verified): unconditional for a HUMAN-mode taker — the intended product behavior per the outline; NO accepted human-mode stream altered (receiver-steering streams' taker player-1 is CPU-driven; HUMAN-KEEPER-CONTROL and 5V5-HUMAN-VS-CPU trajectories have zero restart events; kickoff untouched — verified per family). CPU FALLBACK BYTE-IDENTITY: critic reproduced HEAD-vs-branch identity on the accepted stream shape (ea956e7c…); builder scratch eb308194… identical HEAD vs branch. HUMAN-SERVED TWO-RUN ATTESTATION: a3370fb4… byte-identical across two runs; distinct from both CPU hashes (gate not a no-op, binding-asserted). CONFORMANCE: THROW-IN-SERVE/THROW-IN-TIMER-FREEZE/MATCH-TIMER-FREEZE/MATCH-TIMER-DECREMENT/RESTART-FREEZE-UNTIL-FIRST-TOUCH/KICKOFF-FREEZE PASS on the human-served stream; honest NOT_EVALUATED for KICKOFF-FIRST-TOUCH/RESTART-NEAREST-ONLY/THROW-IN-AWARD/PLACEMENT (driven window); BLOCKED unchanged. Evidence: record_sha256 85fc082d30e9120c5c12f9d9e6950dbb332fee8960affdf8852f8feb5c4734ef (no wall-clock field; reproduced byte-exact independently by critic AND integration; docs/ byte-identical on ordinary runs; durable writes under WIP_SECTION=__EVIDENCE__:HUMAN-BALL-SERVER-LITERAL); audit PASS MULTI_TICK. Tests: typecheck 0; new 3+7+1; registry bindings 101/101; restart battery 158/158; architecture 27/27; core subsystems 490/490; rules-gate 140/140; fouls bindings 54/54; integration ran ~1000+ tests across pins/two-run/replay/canaries/rules/GK/restart/human/browser-parity — NO accepted pin moved (one 6-fail batch under triple-concurrent vitest load re-ran standalone 115/115, harness contention disclosed); gauntlet:eval 39+33; eval:state 13/13. Disclosures: an annotation-only contact pass event on the serve tick (taker within passRadius of the placed ball; the authoritative serve is the match-phase serve, committed velocity exactly dir×12, no double impulse, no contact-system change); vitest batches split under the wrapper timeout; the 90-tick window is a literal in simulation.ts (provenance disclosed; candidate for the match-rules-v1 config surface); the new oracles have no dedicated mutant/canary guards (not required by the outline; PASS/NOT_EVALUATED asymmetry + gate-not-a-no-op binding-covered); the capture script runs via tsx directly (no package.json script yet); a deterministic wait-phase gravity drift z 1.5→1.42 (cosmetic). claims_not_made: no PES fidelity or measured envelope / no invented reference envelope (BLOCKED refs stay blocked) / no suite-level PASS / no DYNAMIC_VISUAL claim (MULTI_TICK) / no in-play human behavior change claimed / no contact-system modification.


## Iteration 195 — 2026-09-07
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 32b540e candidate(GK-DRIVEN-CLOSURE)
- notes: Horizon v34 4/4 — horizon COMPLETE. The two per-stream goalkeepers gaps closed with driven streams (BOOKKEEPING; zero source change — git diff src/ src/contracts/ EMPTY, no evaluator/oracle/contract/adapter/runner/spec change; verified by builder, critic, AND integration; 4 additive paths). STREAMS: (a) gk-save-driven reuses the accepted 5v5-keeper-shot-fixture.v1.json — 4 real save/claim chains within save_claim_reach_radius 1.2 m (shot@361→contact@362 1.081 m; @370→@374 0.737 m; @376→@380 0.551 m; @382→@386 0.473 m; gaps 1–4 ticks ≤ reaction 12); GK-SAVE-CLAIM PASS on the stream. (b) gk-release-driven uses the new 5v5-keeper-release-fixture.v1.json (fixture purity verified: empty inputProgram/scheduledEvents, adapter initial state only, all AI_FALLBACK, no teleports) — 12 keeper-release events (ticks 1,4,7,10,13,42,56,58,61,64,67,70), every target the observed forward player-9 (no omniscience; the adapter's own KeeperReleaseRecord distribution path); GK-DISTRIBUTION-NO-OMNISCIENCE PASS on the stream. Both two-run byte-identical. BASELINE-CONFLATION HANDLED (critic adjudicated honest; the v31 conflation precedent): the horizon text labeled 9/0/2/1/1 as the GK-SUITE-CORE-OWNED-STATE baseline, but 9/0/2/1/1 is the SUITE-DETERMINISTIC-TWO-RUN post-two-run table (abaf6ccd…) where GK-SAVE-CLAIM and GK-DISTRIBUTION-NO-OMNISCIENCE are already PASS; GK-SUITE-CORE-OWNED-STATE (5cd1c808…) is 8/0/3/1/1 single-run; the 2 aggregate NOT_EVALUATED rows are GK-*-REG/GK-*-CAUSAL (must not be converted). The horizon text corrected in place during this iteration's bookkeeping; the durable record discloses the split verbatim (baseline.note + disclosure #4). Aggregate verdict_counts honestly UNCHANGED at 9/0/2/1/1 (delta_vs_baseline_count_change.changed = false); the real closure value is the per-stream evidence where it was actually missing. Evidence class BOOKKEEPING (accurate per gauntlet/evidence-classes.md: deterministic suite-state publication over existing + new MULTI_TICK streams; the trajectory attestation is embedded as per-stream state-hash chains + two-run identities; the audit's required-trajectory check NOT_APPLICABLE). Evidence: record_sha256 21a596277aac626f3612e225ced4145310e1a81631bd28552f9188d223b33d7a (no wall-clock field; reproduced byte-exact independently by critic AND integration, both streams re-executed live; docs/ byte-identical on ordinary runs; durable writes under WIP_SECTION=__EVIDENCE__:GK-DRIVEN-CLOSURE); audit PASS BOOKKEEPING. Tests: typecheck 0; binding 9/9 (incl. the not-hand-written live reproduction through runHeadlessMatch + evaluateSuite); GK/stateHash/rules battery 196/196 (15 files); evaluator/hygiene 153/153; critic spot 42/42; integration GK battery 131 + rules/registry/hygiene 179; gauntlet:eval 39+33 (the new eval/scenarios fixture is consumed-by-path only — the gauntlet gate reads gauntlet/evals/scenarios/ and stays 39; explicitly resolved); eval:state 13/13. claims_not_made: no suite-level PASS / no reference-target invention (GK-*-REF blocked, GK-*-VIS perceptual, reg/causal NOT_EVALUATED) / no gameplay or evaluator change / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity / no accepted-record mutation / no aggregate-count inflation.


## Iteration 196 — 2026-09-08
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 83622cb candidate(FOUL-CONSEQUENCE-MACHINERY)
- notes: Horizon v35 1/4. A detected foul awards a free-kick restart to the fouled team at the contact position through the ACCEPTED restart machinery (MULTI_TICK; the HUMAN-BALL-SERVER-LITERAL named-conformance-path pattern). CORE: simulation.ts +349 — default-OFF awardFreeKicks closure gate (optional 8th createSimulation arg; all pre-existing call sites default-off); 6b-4 award branch (only while "playing") with findFoulContact on the core's committed contacts; onFreeKickEvent (award/place/taker=closest fouled-team player/receivers/markers; lastTouchRef nulled); applyFreeKick/applyFreeKickFromInput through the accepted serve path; a free-kick countdown-zero branch mirroring the accepted branches with the UNCHANGED HUMAN_SERVE_WAIT_WINDOW_TICKS=90 human-serve window; versioned-provisional constants (countdown 60 / speed 14 / vz 0.18 / z 0.11). SINGLE SOURCE OF TRUTH: src/simulation/foul-predicate.ts shared by the accepted detection runner (refactored to import it, re-exporting FOUL_CONTACT_TYPES) and the consequence — predicate equivalence proven at source (duelWon = ballReachable at tackle-system.ts:505; both fields committed in every payload :532/:558; the only other contact producer emits contactType "player-player" with no duelWon and is excluded identically). Contracts: state.ts +37 optional WorldState fields + MatchPhase "free-kick"; scenario.ts +1 "free-kick-executed"; adapters phase-union wiring only. BYTE-IDENTITY: gate-off reproduces fb5e9b02… byte-exact; no-tackle-additivity 5 shapes byte-identical; the ACCEPTED FOUL-DETECTION record 69104ea1… reproduces byte-exact under the refactored runner (file 0c1c3b54… both sides); all pinned rules/GK tables reproduce. TWO-RUN ATTESTATION: driven 5f3bc337… (foul tick 51 → FK tick 111 = 51+60) and organic 47674adf… (foul 233 → FK 293), run1=run2. HONEST RE-EVALUATION: organic stream TIMER-DECREMENT + TIMER-FREEZE PASS; RESTART-FREEZE-UNTIL-FIRST-TOUCH NOT_EVALUATED and MATCH-RESTART-NEAREST-ONLY genuine FAIL on the anti-huddle-OFF organic shape (the unmodified oracle's true verdict, disclosed); driven anti-huddle window FREEZE-UNTIL-FIRST-TOUCH PASS, NEAREST-ONLY/REARM NOT_EVALUATED (bound to other restart kinds); per-tick timer freeze attested in the sim test. Same-tick arbitration: deterministic out-of-play-wins priority, disclosed (spec §2.2 omits the matrix). Evidence: record_sha256 39da80ad4106546939afeefa34cca2763a113bdfc559eec013a9ee578834da95 (no wall-clock field; reproduced byte-exact independently by critic AND integration; docs/ byte-identical on ordinary runs; WIP_SECTION=__EVIDENCE__:FOUL-CONSEQUENCE-MACHINERY); audit PASS MULTI_TICK. Tests: typecheck 0; new 4+7+1; no-tackle-additivity 5; neighbors 197 eval + 75 integration (builder); 203 node files / 3,930 tests + 52 browser files / 268 tests (integration, all green, NO accepted pin moved); gauntlet:eval 39+33; eval:state 13/13; build ok. Disclosures: the driven-duel free-kick-executed lives in core persistent state not the per-step observations (runner serialization limit; surfaced as committed_free_kick_count); the renderer renders "FREE-KICK" via the toUpperCase fallback (curated label deferred to 4/4); a stale "gate off" comment in the non-human countdown path (cosmetic, fix opportunistically in 2/4); the freeKickWindow control opens the window without a foul (distinct from the attested chain); worker-RPC artifacts non-fatal. claims_not_made: no card/advantage implementation / no FREE-KICK-AWARD registration (2/4) / no suite-level PASS / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity / no ungated behavior change / no DYNAMIC_VISUAL claim.


## Iteration 197 — 2026-09-08

- objective_id: FREE-KICK-SUITE-REGISTRATION
- builder: builder-structured / deepseek-v4-flash
- critic: critic / glm5.3-flash — ACCEPT (first pass)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 1eec4eb candidate(FREE-KICK-SUITE-REGISTRATION)
- notes: Horizon v35 2/4. FREE-KICK-AWARD (FOULS_CARDS_SPEC §10 — "the set-piece consequence of a called foul") registered as the third executable protected oracle in suite-fouls-v1 over the accepted FOUL-CONSEQUENCE-MACHINERY streams (MULTI_TICK; zero gameplay change — git diff src/ src/contracts/ eval/runners/ EMPTY, verified by builder, critic, AND integration; all additions, zero pre-existing rows changed). CHAIN: FREE_KICK_AWARD (HARD_INVARIANT) → INV_FOUL_FREE_KICK_AWARD → foul-free-kick-award-oracle-v1 (checkFoulFreeKickAward importing the SHARED src/simulation/foul-predicate.ts) → wire.ts → CRITERION_TO_ORACLE → BINDING_FOULS_FREE_KICK_AWARD_001 → FOULS-FREE-KICK-AWARD-001 → EXPANSION_FOULS_V1. Registry: 59 bindings / 96 criteria / 39 invariants / 8 suites; content_hash fnv1a64-v1:44ebd3e8c224d6e7 (independently computed by critic AND integration via loadRegistrySet; validation 0 errors; no literal pin — the provenance format accommodation). GUARDS per §10's definition: FK-without-foul → FAIL (both power-guard controls 63063b34… anti-huddle-window and 428a4df5… human-serve FAIL); foul-without-observable-FK → honest NOT_EVALUATED (driven 5f3bc337… commits in core state; the serialization limit not worked around); no-foul/no-FK → NOT_EVALUATED (gate-off fb5e9b02…); wrong-team / placement >0.5 m → FAIL. EXECUTED VERDICTS: organic 47674adf… (foul 233 → FK 293 in observations) PASS; driven NOT_EVALUATED; both power-guards FAIL; gate-off NOT_EVALUATED. All five stream state_hash_of_hashes values character-identical to the accepted pins; the accepted FOUL-CONSEQUENCE record 39da80ad… re-verified byte-exact. SPEC OPEN THREAD RESOLVED: §10 heading/wording → "three registered, two named-not-registered" (CARD-ISSUED + ADVANTAGE-PLAYED stay NAMED-NOT-REGISTERED, grep-verified + binding-enforced; the no-PASS rule kept and re-scoped; the "NOT registered" and "No PASS may be reported" phrase assertions still green; fouls-spec-binding updated with FREE-KICK-AWARD → REGISTERED_CRITERIA, disclosed). Evidence: record_sha256 86a34acb5f7336ac339c91fced93246c52f1f6beadb5407ae916cae7e6b425f2 (no wall-clock field; reproduced byte-exact twice by the critic, re-reproduced by integration; docs/ byte-identical on ordinary runs; WIP_SECTION=__EVIDENCE__:FREE-KICK-SUITE-REGISTRATION); audit PASS MULTI_TICK. Tests: fouls-suite-binding 24/24 (three new canary guards), fouls-spec-binding 42/42, eval-registry 48/48 (59 bindings), oracle-registry 19/19, foul-detection 12/12 + foul-consequence 7/7 + sim 4/4; integration 28 registry-consumer files / 523 node tests + 5 neighbors all green; typecheck 0; gauntlet:eval 39+33; eval:state 13/13. The 0.5 m tolerance judged disclosed evaluator-side slack (both sides derive from the same committed field; measured delta 0; the rules-suite 0.2 m precedent class; tighten at the next natural touch). Disclosures: no suite-level PASS (3 of 5 registered); two stale cosmetic test comments; the spec §2.1/§2.2 prose staleness pre-dates this candidate (fold into the next natural fouls objective). claims_not_made: no card/advantage machinery or verdicts / no suite-level PASS / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented envelope / no gameplay, runner, or consequence-machinery change.


## Iteration 198 — 2026-09-08

- objective_id: GK-REGRESSION-POLICY-REGISTRATION
- builder: builder-structured / deepseek-v4-flash
- critic: critic / glm5.3-flash — RETRY then ACCEPT
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 5dd2c1e candidate(GK-REGRESSION-POLICY-REGISTRATION)
- notes: Horizon v35 3/4. The gk-regression canary (gk-regression-canary-v1) registered, converting the six GK-*-REG criteria from honest NOT_EVALUATED to executed PASS over the accepted + driven GK streams (gk-continuous-live 1800 ticks / gk-shot-fixture-live 600 / gk-release-fixture-live 300 — 18 executed PASS verdicts; MULTI_TICK; zero gameplay change — git diff src/ src/contracts/ EMPTY). CHAIN: the six GK-*-REG criterion_bindings [] → ["gk-regression-evidence"] → INV_GK_REGRESSION → gk-regression-canary-v1 → six CRITERION_TO_ORACLE entries + computeOutcome(REGRESSION) judges on the registered oracle (the hunk isolated to the REGRESSION branch; empty oracleResults → NOT_EVALUATED — pass-by-silence impossible). NON-CIRCULARITY (critic's explicit judgment): the baseline is a fixed committed set — the hard-coded §9 spec values (cross-checked against GOALKEEPER_SPEC §9, goalkeeper-config.ts, goalkeeper-role.ts GK_SMALL_SIDED_V1), the version id gk-small-sided-v1, the accepted protected oracles (thresholds from versioned GK_PROVISIONAL_VALUES, not the stream under test); the hash check is integrity-only (non-empty + strictly monotonic ticks). POWER: off-arc mutant FAIL; divergent config (radius 3.5, no bump) FAIL; empty stateHash FAIL; duplicate tick FAIL; non-GK stream NOT_EVALUATED; version bump → NOT_EVALUATED (never a silent re-pin); any single pin diverging flips all six GK-*-REG to FAIL. CRITIC RETRY (a legitimate catch): the original §11.2 prose promised a beyond-the-reaction-window FAIL that the accepted checkGkSaveClaim oracle does not enforce (its search breaks at the window boundary — a late save or an unanswered shot returns not_evaluated while the canary would still PASS). FIX: option 2, correct-the-prose (disclosed as the TEXT-ENFORCEMENT CORRECTION) — implementing a late/missing-save FAIL would require weakening the accepted oracle (forbidden) and would false-positive on legitimately scored shots; the corrected semantics (spec §11.2 + the oracle doc header + the record's pins_guarded.save_claim): in-window + in-reach = preserved pin (pass); in-window + out-of-reach = FAIL; beyond-window or unanswered = honest NOT_EVALUATED; the accepted oracle untouched (0-line diff); four new binding guards pin the late/missing cases as NOT-FAIL + two record assertions (the save_claim text locked; keeper_marker_baseline_sha asserted as referenced provenance, NOT an enforced pin). NORMATIVE SOURCE: option (b) spec-prose addition — NEW versioned GOALKEEPER_SPEC.md §11.2 "Regression policy (GK-*-REG)" (the six criteria, the pins, the version-bump rule; refuses to convert REF/VIS/CAUSAL; no PES constants; GAMEPLAY_EVALUATION_SPEC §5.5's comparison_condition_hash machinery stays unoperationalized — disclosed). VERDICTS: GK-*-REF BLOCKED_MISSING_REFERENCE + GK-*-VIS NEEDS_PERCEPTUAL_REVIEW + GK-*-CAUSAL NOT_EVALUATED untouched (CAUSAL verified a different criterion — UNKNOWN class); TACK-*/INT-*/PHY-*-REG stay NOT_EVALUATED (direct probes). Registry: content_hash fnv1a64-v1:44ebd3e8c224d6e7 → fnv1a64-v1:e31411fbf274f66f (the critic computed the before-hash independently in a temp worktree; ALL_TEST_IDS stays 59). Evidence: record_sha256 fc7d1b97b3b1a78613b571610e3bdbc53aa076b5ae42f828555a4110e037e60e (no wall-clock field; reproduced byte-exact by the critic after the RETRY refresh; durable writes under WIP_SECTION=__EVIDENCE__:GK-REGRESSION-POLICY-REGISTRATION); audit PASS MULTI_TICK. Tests: binding 42/42; integration 103 (registry) + 102 (duels/GK) + 58 (GK state) + 152 (fouls/rules/restart) + 131 (two-run/hygiene/canaries/replay/lifecycle) — all green, NO pinned verdict moved beyond the six intended conversions; the accepted GK records byte-untouched (provenance pins 5cd1c808…, 21a59627… match on disk); typecheck 0; gauntlet:eval 39+33; eval:state 13/13; docs/ byte-identical on ordinary runs. Disclosures: the STREAM-COVERAGE CAVEAT (a single stream as sole REG evidence can leave a pin silently unobservable — the three streams collectively cover all pins); the vitest onTaskUpdate worker-RPC artifact on heavy batteries (non-fatal, all tests pass in every configuration). claims_not_made: no suite-level PASS / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented envelope (wrong-foot/parry/recovery/crosses NOT claimed as measured) / no gameplay change / no accepted-record mutation.


## Iteration 199 — 2026-09-08

- objective_id: FOUL-FREEKICK-BROWSER-EVIDENCE
- builder: builder-gameplay / deepseek-v4-flash (reroute)
- critic: critic / glm5.3-flash — ACCEPT (first pass)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: bba7bef candidate(FOUL-FREEKICK-BROWSER-EVIDENCE)
- notes: Horizon v35 4/4 — horizon COMPLETE. The foul → free-kick consequence visible in the real browser (DYNAMIC_VISUAL, audited at the named class; zero gameplay change — git diff src/simulation/ src/contracts/ EMPTY; the only src changes are presentation/harness: the renderer's curated "FREE KICK" label case (gated behind the off-by-default showMatchPhaseHud — the label the 1/4 critic deferred here) and the test-bridge optional freeKickConfig passthrough (default undefined; ~125 pre-existing createTestBridge call sites across 33 files pass at most 4 positional args — consumer-enumerated; main.ts unchanged)). THE DRIVE: real Chromium via test-bridge + the real Three renderer (showMatchPhaseHud:true) over the accepted FOUL-CONSEQUENCE driven-duel shape (5v5-human-vs-cpu + withProximateHumanDefence; the accepted defensive-duel scripted standing tackle; freeKickConfig:{awardFreeKicks:true}) — real core events via the shared foul-predicate. FRAMES (4 byte-distinct PNGs over the captured bytes): tackle-contact@50 (36ea577e…) → freekick-award@51 (cf522fae…, the curated FREE KICK label, the ball re-placed at the contact spot) → freekick-placement@91 (f31f80e0…, TIME frozen 5350; pixel-difference forensics: only two small player-drift regions, the ball region pixel-unchanged → stationary) → freekick-served@114 (f1bbe9b8…, PLAYING, TIME 5346 — the remaining-ticks countdown resumed at the tick-111 serve, correct frozen-restart semantics); sequence.json path/tick/semantic/sha256 bindings 4/4 verified against the actual PNG bytes. CORRESPONDENCE: browser foul@51 → FK@111 (team-b, contact (−1.813, 0.007)) vs headless identical → offset 0/0 EXACT (critic reproduced three times). Evidence: record_sha256 e65c3618369a1474eb2c094714b41fb85986e68d3a6e6f19011fd7ed4cb6188c (no wall-clock field; reproduced byte-exact by the critic twice AND integration; docs/ byte-identical on ordinary runs — 1045 files; WIP_SECTION=__EVIDENCE__:FOUL-FREEKICK-BROWSER-EVIDENCE); audit PASS DYNAMIC_VISUAL (20 checks incl. screenshot-SHA-uniqueness). Tests: browser capture 3/3 + integration 2/2; typecheck 0 + build 0; byte-neutrality via keeper-visual-marker 4/4 (the byte-identity parity guard) + fulltime-flow-closure 7/7 + archetype-identical-recapture; the accepted bindings green (fouls-suite 24 + foul-consequence 12 + foul-detection 12 + fouls-spec 42); integration 32 browser files / 253 tests all green (8 pure-capture HUD-off suites uncovered within the 300 s budget — the byte-identity risk surface covered by keeper-visual-marker + archetype-identical-recapture; disclosed) + node-side 92+50+48+state bindings green; gauntlet:eval 39+33; eval:state 13/13. Disclosures: driven duel (not organic); free-kick-executed in core persistent state (accepted serialization limit); the free-kick countdown/speed/loft/z VERSIONED_PROVISIONAL; the capture-foul-freekick-browser-record package script self-sets WIP_SECTION (durable-by-design); the pre-existing onTaskUpdate artifact (one spurious fail-mark on GK-REGRESSION-POLICY-REGISTRATION in a combined run re-ran solo 42/42). claims_not_made: no card/advantage implementation or evidence / no FREE-KICK-AWARD registration here / no suite-level PASS / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity / no per-tick float identity claim across runtimes / no gameplay or consequence-machinery change.


## Iteration 200 — 2026-09-08
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 7fc7969 candidate(FOULS-AGGREGATE-HONESTY-RERUN)
- notes: Horizon v36 1/4. The honest aggregate verdict state re-published after the GK-*-REG conversion + the cosmetic threads folded (BOOKKEEPING, the RULES-SUITE-STATE-RERUN pattern; zero gameplay/source change — git diff src/ src/contracts/ EMPTY). AGGREGATES: goalkeepers with the registered canary live → 10/0/1/1/1 — exactly ONE criterion changed vs the 9/0/2/1/1 baseline (abaf6ccd… loaded verbatim and diffed by the critic; integration recomputed independently): reg NOT_EVALUATED → PASS through the registered gk-regression canary (the critic independently re-evaluated gk-release-fixture-live: all six GK-*-REG PASS); COMMON-DETERMINISTIC carried by the real two-run byte-identity attestation; the 9 behavior/common keys unchanged; ref/vis/causal untouched. Fouls suite: 3 of 5 registered, all executed PASS (FOUL-DETECT + FOUL-CLEAN-TACKLE on four streams; FREE-KICK-AWARD on freekick-organic); the two power-guard streams deliberately excluded from the genuine aggregate AND reported as executed FAILs (the eligibility-exclusion precedent); CARD-ISSUED/ADVANTAGE-PLAYED stay named-not-registered; no suite-level PASS. COSMETIC FOLD (verdict-neutral, stash-verified both directions by the critic): the FREE_KICK_POSITION_TOLERANCE prose disclosure (8 JSDoc lines, doc-only); fouls-suite-binding:45 comment; the foul-consequence binding test TITLE (assertion untouched); FOULS_CARDS_SPEC §2.1/§2.2 → the accurate 3-registered/2-named-not-registered state (fouls-spec-binding 42/42 green before AND after; no test pins the new wording — substring assertions unaffected); package.json +2 capture-script registrations reproducing the accepted record SHAs (85fc082d…, 39da80ad…; the browser-frames name already taken — the record producer stays under capture-foul-freekick-browser-record, disclosed). Evidence: record_sha256 c8b3b63edd57f6acd15e91f4daa0bdb8d35e239155ee51b6975cc76f59acaa21 (no wall-clock field; reproduced byte-exact by critic AND integration; docs/ byte-identical on ordinary runs; WIP_SECTION=__EVIDENCE__:FOULS-AGGREGATE-HONESTY-RERUN); audit PASS BOOKKEEPING. Tests: the new binding 9/9; 82 + 175 + 76 + 51 + 31 + 89 battery tests green; typecheck 0; gauntlet:eval 39+33; eval:state 13/13. Disclosures: the residual FOULS_CARDS_SPEC Status/Scope preamble staleness pinned verbatim by fouls-spec-binding:83 — left in place (conservative, under-claims) to fold at CARD-ISSUED-SUITE-REGISTRATION; two stale header comments outside the scoped lines (cosmetic); the no-verdict controls cited from accepted records (runtime bounding; cannot affect any aggregate); the vitest worker-RPC artifact non-fatal. claims_not_made: no suite-level PASS for either suite / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity / no invented envelope / no criterion upgraded beyond the executed evaluator / no gameplay/source/contract change.


## Iteration 201 — 2026-09-08
...
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 081c927 candidate(CARD-MACHINERY)
- notes: Horizon v36 2/4. Card issuance per FOULS_CARDS_SPEC's card definition over the accepted foul machinery (MULTI_TICK; a gated deliberate core change in the named-conformance-path pattern). SPEC GATE (the critic re-read §5.1/§6/§7/§9.1/§10/§11 verbatim, judgment: correct and complete): IMPLEMENTED — exactly the §7 accumulation path (per-player per-match card accumulation; a versioned provisional count before a caution (yellow) and before an expulsion (red)) with §9.1's fouls-v1 VERSIONED_PROVISIONAL pins 2/5 pinned exactly in the NEW src/simulation/card-policy.ts (FOULS_YELLOW_ACCUMULATION_COUNT=2 / FOULS_RED_ACCUMULATION_COUNT=5 / resolveCardForAccumulatedFouls — the card-threshold single source of truth); the offender is the tackler (playerIdA/teamIdA per tackle-system.ts:546); the card event (cardType, offender, fouledPlayerId, accumulatedFouls, foulSourceEventId, foulTick) committed at the 6b-3c branch BEFORE the free-kick branch (a card and a free kick coexist on the same foul tick — attested) and only while "playing"; the booking lazy-materialized (state.bookings ??= {} only on a qualifying foul; optional WorldState fields). BLOCKED_MISSING_REFERENCE surfaced with ZERO implementation: direct-red-by-contact-severity (§9.1's 0.85 threshold has no defined severity computation and the accepted contacts carry no severity field; §11 refs BLOCKED) and second-yellow→red (§7 defines two INDEPENDENT thresholds). ADVANTAGE-PLAYED stays spec-only (§6). CORE: simulation.ts +108 (CardConfig{issueCards} default-off; the optional 9th createSimulation param — 131 call sites enumerated by the integration reviewer, every pre-existing call ≤2 args default-identical, main.ts unchanged); scenario.ts +1 card-issued (grep-verified absent from every other accepted stream); state.ts +33 (PlayerBooking + optional bookings); headless-match.ts +25 (issueCards default false, hash-neutral gated off); defensive-duel-driver.ts +30 (cardEvents + bookingState). BYTE-IDENTITY: the gate-off pin fb5e9b02… reproduced byte-for-byte (critic AND integration); the card-driven stream two-run a53e1827… byte-identical. RESULTS: card-driven-duel caution@2 (tick 116) + expulsion@5 (tick 371), booking player-1 {fouls:5, cautions:1, expulsions:1}; card-combined caution@2 (tick 290) + 2 committed free kicks coexisting; card-organic 1 foul / 0 cards (below threshold, honest); card-gate-off 0/0. RE-EVALUATION: FOUL-DETECT + FOUL-CLEAN-TACKLE PASS on the three live card streams; FREE-KICK-AWARD honest NOT_EVALUATED; CARD-ISSUED + ADVANTAGE-PLAYED not claimed; no registration (3/4); no suite-level PASS. Evidence: record_sha256 01d731ada16b2451b1a939bf9d20af66da8bdc07445fbcd150cd41062a2cba66 (no wall-clock field; reproduced byte-exact by the critic (full JSON diff) AND the integration reviewer three ways; docs/ byte-identical on ordinary runs — 1,060 files; WIP_SECTION=__EVIDENCE__:CARD-MACHINERY); audit PASS MULTI_TICK. Tests: new sim 4/4; typecheck 0 + build 0; 631 neighbor tests (determinism 107, rules+fouls 209, GK 154, restart 197, human/loop 133, architecture 27) — zero behavioral regressions, no accepted pin moved; gauntlet:eval 39+33; eval:state 13/13. Disclosures: post-expulsion accumulation (fouls > 5 → no card) is the honest spec-defined-no-further-threshold behavior; card-issued is commit-only via the accepted committed-events injection; the organic booking_state null (runHeadlessMatch returns observations not final state); the trajectory capture_mode distinction; chunked batteries; the worker-RPC artifact non-fatal. claims_not_made: no severity path / no second-card rule / no advantage implementation / no CARD-ISSUED registration or evaluation (3/4) / no suite-level PASS / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity (2/5 are fouls-v1 provisional) / no ungated behavior change / no DYNAMIC_VISUAL claim.


## Iteration 202 — 2026-09-08

- objective_id: CARD-ISSUED-SUITE-REGISTRATION
- builder: builder-structured / deepseek-v4-flash
- critic: critic / glm5.3-flash — ACCEPT (first pass)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 793572c candidate(CARD-ISSUED-SUITE-REGISTRATION)
- notes: Horizon v36 3/4. CARD-ISSUED (FOULS_CARDS_SPEC §10) registered as the fourth executable protected oracle in suite-fouls-v1 over the accepted card streams (MULTI_TICK; zero gameplay change — git diff src/ src/contracts/ EMPTY; card-policy.ts and foul-predicate.ts imported only). CHAIN: CARD_ISSUED (HARD_INVARIANT) → INV_FOUL_CARD_ISSUED → foul-card-issued-oracle-v1 (checkFoulCardIssued importing the SHARED resolveCardForAccumulatedFouls + the shared §5.1 predicate) → wire.ts → CRITERION_TO_ORACLE → BINDING_FOULS_CARD_ISSUED_001 → FOULS-CARD-ISSUED-001 → EXPANSION_FOULS_V1. GUARDS: no-backing-foul FAIL; wrong-player FAIL (card.playerId must equal foul.playerIdA); wrong-type-at-count FAIL (accumulatedFouls must equal the genuine-foul count at foulTick AND resolveCardForAccumulatedFouls(count) === cardType); duplicate-card-per-foul FAIL; no observable card → honest NOT_EVALUATED. HONESTY JUDGMENT (critic, explicit; integration corroborated with a direct runDefensiveDuel probe): all four accepted card streams honestly NOT_EVALUATED — the serializeRestartFacts committed-events injection includes card-issued and is live on the organic stream (the absence is NOT a serialization excuse); the organic shape commits 1 foul < threshold 2 (no card exists to inject); the driven shape exposes cards only via runDefensiveDuel.cardEvents (injection there would be the forbidden runner re-serialization); an executed PASS is unachievable today without a dishonest re-shape; the canaries prove the oracle executable and discriminating (a valid card stream → PASS; each mutation → the correct FAIL). EXECUTED VERDICTS: driven/combined/organic/gate-off all NOT_EVALUATED for CARD-ISSUED (5/2/1/0 fouls; 0 observable cards); FOUL-DETECT/FOUL-CLEAN-TACKLE unchanged (PASS on the three live streams); FREE-KICK-AWARD unchanged; all four streams state_hash_chain_identical_to_accepted = true. ADVANTAGE-PLAYED stays named-not-registered (a direct probe produced no verdict anywhere). SPEC FOLD (anticipated by 1/4): Status preamble ("The engine has foul recognition, the card consequence and the free-kick consequence" / "The engine has NO advantage machinery"), §2.1/§2.2, §7 heading, §10 heading (four registered, one named-not-registered; the no-PASS rule re-scoped to ADVANTAGE-PLAYED), §13; fouls-spec-binding line 83 re-pinned, REGISTERED_CRITERIA = the four, REMAINING = [ADVANTAGE-PLAYED]; every other phrase assertion green (42/42). Registry: 60 bindings / 97 criteria / 41 invariants / 8 suites; content_hash fnv1a64-v1:54743b465ed6ee90 (both reviewers loaded independently; validateRegistrySet 0 errors; no literal pin; eval-registry 59→60 the only numeric pin updated). Evidence: record_sha256 3dac8a48cae826d846e58c16ec487cc40187b645b9b17bc385ae2f64dc16bd3c (no wall-clock field; reproduced byte-exact twice by the critic and twice by the integration reviewer; docs/ byte-identical on ordinary runs — 1,064 files; WIP_SECTION=__EVIDENCE__:CARD-ISSUED-SUITE-REGISTRATION); audit PASS MULTI_TICK. Tests: fouls-suite-binding 30/30 (5 new canary guards), fouls-spec-binding 42/42, eval-registry 48/48; integration 707 registry-consumer tests 0 failures (registry 110, rules 197, GK 139, duels 62, fouls 105, canaries/replay/lifecycle 84, release binding 10); typecheck 0; gauntlet:eval 39+33; eval:state 13/13. Disclosures: no suite-level PASS (4 of 5 registered); the full tests/unit/eval chunked (>300s); the worker-RPC artifact non-fatal; the claim-precision note (non-blocking): the genuine-foul filter at fouls.ts:531 uses the module-local isFoulCandidate (duelWon only) rather than the shared isFoulCandidatePayload (which also checks ballReachable) — semantics coincide under the accepted duelWon ≡ ballReachable invariant and FOUL-DETECT would FAIL a mutated payload, so no guard is materially weakened; the doc header/claim imprecision to align at a future fold. claims_not_made: no suite-level PASS (4 of 5 registered) / no CARD-ISSUED PASS over any accepted stream / no ADVANTAGE-PLAYED machinery, oracle, or verdict / no PROMOTION / no FOUNDATION_LAB_PASS / no PES fidelity or invented envelope / no gameplay, contract, adapter, or runner-serialization change.


## Iteration 203 — 2026-09-08

- objective_id: CARD-BROWSER-EVIDENCE
- builder: builder-gameplay / deepseek-v4-flash (reroute)
- critic: critic / glm5.3-flash — ACCEPT (first pass)
- integration: integration-reviewer / glm5.3-flash — ACCEPT (first pass)
- result: accepted
- commits: 9d8a1a4 candidate(CARD-BROWSER-EVIDENCE)
- notes: Horizon v36 4/4 — horizon COMPLETE. The booking visible in the real browser (DYNAMIC_VISUAL, audited at the named class; zero gameplay change — git diff src/simulation/ src/contracts/ EMPTY). THE CHANGE: test-bridge.ts +10 (the optional cardConfig passthrough mirroring the accepted freeKickConfig precedent) and renderer.ts +88/−3 (the additive gated card-HUD affordance: showCardHud default false, enrichPresentationWithCards() presentation-only post-process, formatCardNotice(), a draw branch reading snapshot.events). ARCHITECTURAL JUDGMENT (the critic, explicit): the capture-path enrichment is legitimate — upstream of the renderer, the renderer reads only a plain PresentationSnapshot, the capture override pre-existed (the keeper-roles precedent), the draw branch reads the existing untouched contract field snapshot.events, no shared mutation, confined to the test-bridge capture path (main.ts untouched). THE DRIVE: real Chromium (showMatchPhaseHud:true, showCardHud:true, issueCards:true) over 5v5-human-vs-cpu + withProximateHumanDefence with the SAME scripted standing-tackle policy the headless driver uses — 5 real man-not-ball contacts → the in-core consequence issues caution@2/expulsion@5. FRAMES (5 byte-distinct PNGs): foul-1-contact@66 (91e601d5…) → caution@116 (9cc756d0…, "YELLOW CARD player-1 (2)") → continued-foul@173 (b592d4e3…) → foul-4-contact@276 (b09bb6b3…) → expulsion@371 (84d8fd29…, "YELLOW CARD player-1 (2)" + "RED CARD player-1 (5)"); sequence.json bindings 5/5 verified. CORRESPONDENCE: EXACT 0/0 (foul ticks [66,116,173,276,371]; booking {fouls:5, cautions:1, expulsions:1}); temporal coherence TIME = 5400 − tick in every frame. Evidence: record_sha256 bc9daf1fff3167eed5e7d14a6c7a739c3210e4fa449268b5bef76de9384069d0 (no wall-clock field; reproduced byte-exact by the critic (producer run + hand-recomputation) AND the integration reviewer; docs/ byte-identical on ordinary runs — 1,074 files; WIP_SECTION=__EVIDENCE__:CARD-BROWSER-EVIDENCE); audit PASS DYNAMIC_VISUAL. Tests: browser capture 3/3; typecheck 0 + build 0; byte-neutrality proven by re-capturing the accepted FOUL-FREEKICK frames through the modified renderer reproducing the EXACT pinned bytes + keeper-visual-parity reproducing its baseline SHA (05e40d01…); INTEGRATION: the FULL browser battery 54 files / 274 tests (nothing skipped) + the FULL node battery chunked (architecture 29, integration 356, unit 1,255, eval 94 files, root 201) — all green, no accepted pin moved; the createTestBridge enumeration (173 sites / 53 files; only the candidate's two 6-arg calls); gauntlet:eval 39+33; eval:state 13/13. Disclosures: the card HUD is capture-path-only (not wired into main.ts — the shipped loop never enables issueCards); the PresentationSnapshot contract carries no booking field and presentation().events is always empty — the enrichment copies state.events via sim.snapshot() into the snapshot's existing events field (no contracts change); the fixture is a DRIVEN duel; the thresholds VERSIONED_PROVISIONAL; the snapshot() deep-clone optimized to render-ticks; the pre-existing onTaskUpdate artifact + one unattributed load flake (1/342, two complete re-runs 342/342); the enrichment replaces (not appends) the events array — safe today because presentation().events is always empty, an append-based form recommended at a future pass. claims_not_made: no suite-level PASS claim here / no direct-red-by-severity (BLOCKED) / no advantage implementation / no PES fidelity / no claim the cards were forced / no per-tick float identity claim / no src/simulation/ or src/contracts/ change / no FOUNDATION_LAB_PASS / no PROMOTION / no regression PASS.
