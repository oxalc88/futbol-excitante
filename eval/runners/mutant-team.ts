/**
 * @module eval/runners/mutant-team
 *
 * MUTANT_TEAM evaluation: runs the implementable mutant suite in a
 * 3v3 (team) context, records per-mutant outcomes, and reduces
 * MUTANT_TEAM_PASS.
 *
 * Implementation:
 ...
 *
 * Reduction:
 *   MUTANT_TEAM_PASS = PASS iff every implementable mutant is both:
...
/**
 * Outcome for a single team mutant evaluation.
 */
export interface MutantTeamOutcome {
...
/**
 * Full MUTANT_TEAM reduction result.
 */
export interface MutantTeamResult {
  /** Registry version. */
  ...
  /** Overall MUTANT_TEAM_PASS verdict. */
...
/**
 * Run the MUTANT_TEAM evaluation.
 *
 * For each implementable mutant:
 *   - Runs clean evaluation in a 3v3 team context → oracle must PASS
 ...
 * @returns MutantTeamResult.
 */
export function runMutantTeam(
  opts?: {
  ...
  // Load the 3v3 team scenario and run clean evaluation.
  const rawScenario = loadTeamScenario();
  ...
  const outcomes: MutantTeamOutcome[] = [];
  ...
  for (const mutant of IMPLEMENTABLE_MUTANTS) {
    ...
    const outcome: MutantTeamOutcome = {
      mutationId: mutant.id,
      description: mutant.description,
      deferred: false,
      executed: false,
    ...
    try {
      ...
      if (mutant.id === "prng-order") {
        // PRNG-order: run clean vs mutant simulation pair in 3v3 team context.
        outcome.cleanResult = {
          id: `clean-prng-order`,
          status: "pass",
          description:
        ...
        // Build a minimal input program for the 3v3 scenario that mirrors
        // the team nature (3 slots per team).
        ...
        outcome.poisonedResult = {
          ...
          description: prngResult.status === "pass"
            ? `PRNG-order mutation detected divergence in 3v3 team context`
  ...
  // --- Reduction ---
  let verdict: MutantTeamResult["verdict"];
  let details: string;
  ...
  if (anyInvalidRun) {
    const invalidRunMutants = outcomes
      ...
      .map((o) => o.mutationId);
    verdict = "INVALID_RUN";
    details = `MUTANT_TEAM_PASS INVALID_RUN: unexecuted mutant(s) — ${invalidRunMutants.join(
      ", ",
    )} were not evaluated`;
    ...
    const failedMutants = outcomes
      ...
      .map((o) => o.mutationId);
    verdict = "FAIL";
    details = `MUTANT_TEAM_PASS FAIL: not all implementable mutants detected — ${failedMutants.join(
      ", ",
    )} failed detection`;
  } else if (!allDeferredNotEvaluated) {
    verdict = "FAIL";
    details = "MUTANT_TEAM_PASS FAIL: deferred mutants did not return NOT_EVALUATED";
  } else {
    verdict = "PASS";
    details = `MUTANT_TEAM_PASS PASS: all ${IMPLEMENTABLE_MUTANTS.length} implementable mutants detected in 3v3 team context; ${DEFERRED_MUTANTS_V1.length} deferred mutants catalogued`;
  }
