...
For every candidate preserve the full 0.8 pipeline from `gauntlet/PROMPT.md`: builder → tests/artifacts → `pnpm run gauntlet:audit` → optional bounded semantic audit on `REVIEW_REQUIRED` → mandatory critic → integration → final gate → candidate snapshot commit → `GAUNTLET_ACCEPTANCE_JSON=... pnpm run gauntlet:acceptance:persist` with objective `manifest.json` → bookkeeping → `pnpm run gauntlet:eval:state` → final acceptance commit → continue. The critic remains mandatory after deterministic/cheap audit success.
