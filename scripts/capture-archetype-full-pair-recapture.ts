/**
 * Harvest archetype full-pair recapture artifacts from browser test
 * stdout output and persist them to evidence and screenshot directories.
 *
 * Pattern matches capture-archetype-browser-artifacts.ts.
 *
 * Usage:
 *   tsx scripts/capture-archetype-full-pair-recapture.ts
 */
...
const EVIDENCE_DIR = "docs/evidence/ARCHETYPE-FULL-PAIR-RECAPTURE";
const SCREENSHOT_DIR = "docs/screenshots/ARCHETYPE-FULL-PAIR-RECAPTURE";
...
try {
  stdout = execSync(
    "npx vitest run tests/browser/archetype-full-pair-recapture.browser.test.ts --project browser",
    {
      cwd: process.cwd(),
      encoding: "utf-8",
      timeout: 120_000,
...
// Write meta.json files to evidence dir
...
// Write meta.json files to screenshot dir
for (const meta of metaEntries) {
  const filePath = join(SCREENSHOT_DIR, `${meta.baseName}-frame-${meta.tick}.meta.json`);
  writeFileSync(filePath, meta.json + "\n");
}
