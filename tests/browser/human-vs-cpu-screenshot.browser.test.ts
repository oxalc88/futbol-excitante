/**
 * @module tests/browser/human-vs-cpu-screenshot.browser.test
 *
 * Captures a screenshot of the Human-vs-CPU match using the test bridge.
 *
 * Verifies:
 *  1. The human-vs-CPU scenario loads with 4 players (1 HUMAN, 3 CPU).
 *  2. CPU adapters advance simulation autonomously.
 *  3. Keyboard adapter is wired for the HUMAN slot.
...
describe("Human-vs-CPU match screenshot capture", () => {
  it("captures human-vs-CPU match after advancing simulation with CPU slots", async () => {
    // Create bridge with human-vs-CPU scenario.
    ...
    // Verify scene has 4 players (human-vs-CPU scenario).
    ...
    (window as unknown as Record<string, string>).__humanVsCpuScreenshotData = capture.screenshot;
    (window as unknown as Record<string, string>).__humanVsCpuMeta = JSON.stringify(
  ...
  it("slot-1 is HUMAN in human-vs-CPU scenario", () => {
    expect(FOUNDATION_SCENARIO_HUMAN_VS_CPU.controlAssignments["slot-1"].mode).toBe("HUMAN");
    expect(FOUNDATION_SCENARIO_HUMAN_VS_CPU.controlAssignments["slot-1"].teamId).toBe("team-a");
    expect(FOUNDATION_SCENARIO_HUMAN_VS_CPU.controlAssignments["slot-1"].controlledPlayerId).toBe("player-1");
  });
});
