/**
 * Browser-mode DYNAMIC_VISUAL capture + parity guard for KEEPER-VISUAL-MARKER.
 *
 * The SMALL-SIDED keeper role (already accepted) was legible only by position.
 * This objective adds a distinct visual kit marker on the designated keeper and
 * proves it through the REAL browser composition root in Chromium (Vite + the
 * Three renderer through the accepted test bridge):
 *
 *   keeper goal-arc hold -> shot on target -> recorded save contact
 *
 * The run is the driven SMALL-SIDED save fixture
 * `eval/scenarios/5v5-keeper-shot-fixture.v1.json` (ten bodies, nothing
 * scripted: the shooting body's own canonical CPU SHOT press).  The keeper
 * designation comes from the production `designateKeeperFromLayout` /
 * `resolveKeeperPlayerId` rule.  The snapshot the renderer consumes carries an
 * additively-added `keeperRole` flag on the designated keepers (via
 * `enrichPresentationWithKeeperRoles`, a presentation-only post-process — the
 * simulation core is untouched); the renderer draws a magenta triangle above
 * the head ONLY when that field is present.
 *
 * Passes:
 *   Pass 1 - play the fixture with the keeper role live and locate the keeper's
 *            event ticks from the run's own event log (no rendering).
 *   Pass 2 - replay the same wiring, render the frames at those ticks WITH the
 *            keeperRole enrichment, and require the per-tick hash chain and the
 *            located arc to be identical.
 *   Pass 3 - the same wiring with `gkBehavior: false`: no keeper designation,
 *            no marker, no arc.  The render at a frame tick is byte-identical
 *            to the pre-change baseline (the discriminating negative).
 *   Pass 4 - hash the PNGs and write `sequence.json` + `trajectory.json`.
 *
 * Parity guards (Test 2 + Test 3):
 *   (a) with `keeperRole` present the marker renders (enriched bytes differ
 *       from un-enriched bytes at the same tick);
 *   (b) with `keeperRole` absent the render is byte-identical to the pre-change
 *       baseline (a SHA-256 literally captured at HEAD before the marker added);
 *   (c) the snapshot contract change is additive (un-enriched snapshots keep no
 *       `keeperRole`, and the contract fixture consumers are unaffected).
 *
 * Durable evidence is written only through the explicit capture command
 * (`WIP_SECTION=__EVIDENCE__:KEEPER-VISUAL-MARKER ...`); an ordinary suite run
...
const OBJECTIVE_ID = "KEEPER-VISUAL-MARKER";
 ...
 * provisional `gk-small-sided-v1` design values; the keeper-station / spread
 * cadences gate the frame selection.  No new rubric.
 */
...
/**
 * Baseline SHA-256 of the PRE-CHANGE renderer output for the 5v5 base fixture
 * at the SAME camera (captured deterministically at HEAD, before the keeper
 * marker was added).  KeeperRole-absent rendering MUST reproduce it — that is
 * the discriminating negative proving the marker is the ONLY delta introduced.
 */
const PRE_CHANGE_BASELINE_SHA = "05e40d01171667d6b7758c280cd22988c6fdddf93ecd13633aa5b933aef8bc68";

/**
 * Static presentation-only framing, identical for every frame so the sequence
 * stays comparable.  It is aimed at the team-b goal (where the fixture's shots
 * land) so the keeper marker is legible; the renderer consumes immutable
...
interface PlayerRecord {
  ...
  cover: boolean;
  keeper: boolean;
  /** Presentation-only keeper kit marker flag (from the enriched snapshot). */
  keeperMarker: boolean;
}
  ...
  for (let i = 0; i < PLAY_TICKS; i++) {
    ...
    const players: PlayerRecord[] = snapshot.players.map((player) => {
      ...
      return {
        ...
        keeperMarker: keepers.has(player.playerId),
      };
    });
    ...
    if (render && label) {
      // KEEPER-VISUAL-MARKER: present the renderer a snapshot enriched with the
      // keeper designation so the kit marker is drawn.  The enrichment is a
      // presentation-only post-process; the simulation is untouched.
      const enriched = enrichPresentationWithKeeperRoles(sim.presentation(), keeperByTeam);
      const capture = await bridge.capture(enriched);
...
// ---------------------------------------------------------------------------
// Frame plan (event-centered around the keeper save)
// ---------------------------------------------------------------------------
...
function framePlan(arc: GkArc): FramePlan[] {
  ...
  return [
    {
      label: "keeper-arc-marker",
      tick: arc.stationTick,
      semantic: "before",
      description: `Keeper kit marker on its goal arc: the designated keeper (${keeperId}) holds its arc with the magenta marker above the head, while the anti-huddle one-presser+cover shape runs and the keeper is never the chaser`,
    },
    {
      label: "shot-on-target",
      tick: arc.shotTick,
      semantic: "event",
      description: `Shot on target: ${arc.shotPlayerId} strikes the ball toward the keeper's goal (projected cross at y=${arc.projectedCrossY}); the keeper (${keeperId}) marker is visible defending the arc`,
    },
    {
      label: "save-contact",
...
/** SHA-256 of a base64-encoded payload's RAW bytes (what a PNG file would be). */
async function sha256OfBase64Bytes(base64: string): Promise<string> {
  const binary = atob(base64.trim());
  const bytes = new Uint8Array(binary.length);
...
// ---------------------------------------------------------------------------
// Capture (DYNAMIC_VISUAL)
// ---------------------------------------------------------------------------

describe("KEEPER-VISUAL-MARKER: browser keeper kit-marker frames", () => {
  it(
    "captures 3 event-centered frames showing the keeper kit marker on the designated keeper",
    async () => {
      ...
      console.log(
        `[keeper-visual-marker] frames ${ticks.join("/")} shot=${arc!.shotTick}` +
      ...
      // Pass 2 - replay the same wiring and render the three frames (enriched).
      ...
      // result: the keeper's own recorded contact, inside reach.
      ...
      // Pass 3 - the same wiring with the keeper role stashed shows no arc and no
      // keeper activity (matches HEAD before any keeper existed).
      ...
      expect(new Set(pngHashes).size, "the frames must be distinct images").toBe(plan.length);
      ...
      const sequence = {
        ...
        semantic_order:
          "keeper goal-arc marker -> shot on target -> recorded save contact",
        durable_capture: DURABLE_EVIDENCE,
        scenario: scenario.id,
        marker: {
          design: "magenta 3-sided cone (triangle) above the head, kit-independent colour 0xff33ff, drawn ONLY when snapshot PlayerPresentation.keeperRole is true",
          channel: "presentation-only snapshot enrichment, never a simulation value",
          absent_rendering: "byte-identical to the pre-change baseline (see parity guard)",
        },
        arc: {
        ...
        reproduction: {
          capture_test: "tests/browser/keeper-visual-marker.browser.test.ts",
          wiring:
            "src/apps/browser/main.ts 5v5 CPU-vs-CPU composition root with CpuObservation.cpuAntiHuddle + cpuDefensiveTackle + gkBehavior, presentation enriched via enrichPresentationWithKeeperRoles",
      ...
      const trajectory = {
        ...
        evidence_class: "DYNAMIC_VISUAL",
        durable_capture: DURABLE_EVIDENCE,
        produced_by: "tests/browser/keeper-visual-marker.browser.test.ts",
        runtime: "Chromium (vitest --project browser) through src/apps/browser/test-bridge.ts",
        driver:
          "browser composition-root 5v5 CPU-vs-CPU wiring (main.ts per-slot CpuAdapter + computeTeamDecision) under the accepted anti-huddle switch plus gkBehavior; keeper designation recorded through the same exported production function the adapters act on (designateKeeperFromLayout / resolveKeeperPlayerId); the rendered snapshot is enriched with keeperRole via enrichPresentationWithKeeperRoles",
        scenario: {
          id: scenario.id,
          path: SCENARIO_PATH,
        ...
        marker: {
          field: "PlayerPresentation.keeperRole",
          design: "magenta triangle above the head (3-sided cone), colour 0xff33ff, kit-independent; drawn only when keeperRole is present",
          present_players: Object.entries(keeperByTeam).map(([teamId, playerId]) => ({ teamId, playerId })),
          absent_rendering: "byte-identical to the pre-change baseline (parity guard SHA reproduced in test)",
        },
        thresholds: {
        ...
        keeper_metrics: {
          ...
          live: { hold: getKeeperHoldActivations() },
          stashed: { hold: 0 },
        },
        determinism: {
        ...
        stashed_control: {
          ...
          note:
            "with the keeper role stashed no keeper designation exists, so the arc never locates and every keeper-path counter stays 0 - the shape HEAD emitted before any keeper existed; no keeperRole is set, so no marker is drawn",
        },
        cross_runtime: {
          ...
          note:
            "the keeper designation and the shot -> keeper-contact chain structure reproduce the pinned Node read; committed per-tick hashes are NOT compared across runtimes (known pinned-runtime gap)",
        },
        video: {
          status: "NOT_PRODUCED",
          reason:
        ...
        player_flag_legend: {
          ...
          m: "keeperMarker - the designated keeper carrying the presentation-only keeperRole flag",
        },
  ...
  it("keeperRole flag is present ONLY on the designated keepers and absent on non-keepers", async () => {
    const scenario = await loadScenario();
    const bridge = createTestBridge(container, scenario, undefined, {
      ...DEFAULT_RENDERER_CONFIG,
      cameraPosition: CAMERA.position,
    ...
    await bridge.reset();
    const base = bridge.getSimulation().presentation();
    const keeperByTeam = keeperByTeamFromLayout(scenario);
    const enriched = enrichPresentationWithKeeperRoles(base, keeperByTeam);

    // (c) additive: the base (un-enriched) snapshot carries no keeperRole at all.
    expect(base.players.some((p) => p.keeperRole !== undefined)).toBe(false);
    // Enriched: exactly the designated keepers carry keeperRole; everyone else absent.
    const flagPlayers = enriched.players.filter((p) => p.keeperRole === true);
    expect(flagPlayers.map((p) => p.playerId).sort()).toEqual(
      Object.values(keeperByTeam).sort(),
    );
    for (const p of enriched.players) {
      if (p.keeperRole === true) {
        expect(keeperByTeam[p.teamId]).toBe(p.playerId);
      } else {
        expect(p.keeperRole).toBeUndefined();
      }
    }
    // Only the presentation surface changed: tick/geometry/timers are untouched.
    expect(enriched.tick).toBe(base.tick);
    expect(enriched.ball).toBe(base.ball);
    expect(enriched.players.length).toBe(base.players.length);
    bridge.getPresentationSession().dispose();
  });

  it(
    "parity guards: marker renders (a) and keeperRole-absent rendering is byte-identical to the pre-change path (b)",
    async () => {
      const scenario = await loadScenario();
      const bridge = createTestBridge(container, scenario, undefined, {
        ...DEFAULT_RENDERER_CONFIG,
      ...
      // (b) Un-enriched render at tick 0 must reproduce the PRE-CHANGE baseline.
      const unenriched = await bridge.capture();
      const unenrichedBase64 = unenriched.screenshot.split(",")[1] ?? "";
      expect(unenrichedBase64.length).toBeGreaterThan(100);
      const unenrichedSha = await sha256OfBase64Bytes(unenrichedBase64);
      expect(unenrichedSha, "keeperRole-absent render must be byte-identical to the pre-change path").toBe(
        PRE_CHANGE_BASELINE_SHA,
      );

      // (a) Enriched render at the same tick adds the keeper marker -> differs.
      const keeperByTeam = keeperByTeamFromLayout(scenario);
      const enrichedSnapshot = enrichPresentationWithKeeperRoles(
        bridge.getSimulation().presentation(),
        keeperByTeam,
      );
      const enriched = await bridge.capture(enrichedSnapshot);
      const enrichedBase64 = enriched.screenshot.split(",")[1] ?? "";
      const enrichedSha = await sha256OfBase64Bytes(enrichedBase64);
      expect(enrichedSha, "the keeper marker must render and change the pixels").not.toBe(
        unenrichedSha,
      );

      console.log(
        `[keeper-visual-marker] parity unenriched=${unenrichedSha} enriched=${enrichedSha}`,
      );
      bridge.getPresentationSession().dispose();
    },
    120_000,
  ...
  it(
    "sequence.json is byte-coherent with the PNGs and the trajectory anchors",
    async () => {
