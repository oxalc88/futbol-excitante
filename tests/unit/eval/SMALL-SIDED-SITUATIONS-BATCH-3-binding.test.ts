/**
 * @module tests/unit/eval/SMALL-SIDED-SITUATIONS-BATCH-3-binding.test.ts
 *
 * Evidence-binding test: proves that the persisted batch-3 evidence
 * (`docs/evidence/SMALL-SIDED-SITUATIONS-BATCH-3/situations/`) matches
 * a fresh evaluator run (live honesty) against the EXTENDED situation-driven
 * fixture (`3v3-situation-driven-extended.v1.json`).
 *
 * Verifies:
 *  1. All eight situation artifacts exist in batch-3.
...
const BATCH_3_DIR = join(
  dirname(fileURLToPath(import.meta.url)),
  "../../../docs/evidence/SMALL-SIDED-SITUATIONS-BATCH-3/situations",
);

/** All 8 situations covered by the extended fixture */
const BATCH_3_TARGETS = [
  "PASS_RECEPTION",
  "SHOT_TO_RESULT",
  "PHYSICAL_DUEL",
...
beforeAll(() => {
  tmpDir = join("/tmp", `batch3-binding-${Date.now()}`);
...
describe("BATCH-3 binding: persisted artifacts exist", () => {
  it("index.json exists in batch-3 dir", () => {
    const indexPath = join(BATCH_3_DIR, "index.json");
  ...
  it("all eight target situation artifacts exist", () => {
    ...
    for (const target of BATCH_3_TARGETS) {
      expect(
        existsSync(join(BATCH_3_DIR, `${target}.json`)),
  ...
  it("each target artifact's verdict matches the index", () => {
    const index = JSON.parse(
      readFileSync(join(BATCH_3_DIR, "index.json"), "utf-8"),
    );

    for (const target of BATCH_3_TARGETS) {
      const artifact = JSON.parse(
        readFileSync(join(BATCH_3_DIR, `${target}.json`), "utf-8"),
...
describe("BATCH-3 binding: honest verdicts", () => {
  ...
  it("SETTLED_ATTACK_VS_DEFENCE: PASS — all required + indicative shot present", () => {
    const artifact = JSON.parse(
      readFileSync(join(BATCH_3_DIR, "SETTLED_ATTACK_VS_DEFENCE.json"), "utf-8"),
    ) as SituationEvidenceArtifact;
 ...
 * immutable before-state, asserted here to prove the fix's candidate never
 ...
 *
 * The fix makes a settled ball integrate the impulse a touch or a ground pass
 * applies (before it, that ball carried the impulse and never moved), so every
 * per-tick trajectory hash of this fixture moves from tick 1 on. The accepted
 * honest verdict, the relevant event kinds and the relevant event counts are
 * still asserted to reproduce from the live run below, so the binding remains a
 * live claim about gameplay rather than only about bytes.
 */
const BATCH_3_SITUATION_PINS: Record<string, SituationPin> = {
  PASS_RECEPTION: {
...
/** Digest over the artifact's ordered per-tick trajectory hash chain. */
const BATCH_3_TRAJECTORY_PINS: Record<string, SituationPin> = Object.fromEntries(
  Object.keys(BATCH_3_SITUATION_PINS).map((target) => [
    target,
    {
...
const BATCH_3_INDEX_PIN: SituationPin = {
...
describe("BATCH-3 binding: byte-identical re-run", () => {
  it("the durable accepted artifacts are byte-identical to their pinned before-state", () => {
    // docs/evidence is immutable: this objective records the fix's effect on the
    // live re-run and never rewrites the accepted artifact.
    for (const target of BATCH_3_TARGETS) {
      const acceptedContent = readFileSync(
        join(BATCH_3_DIR, `${target}.json`),
        "utf-8",
      );
      expect(
        digestArtifact(acceptedContent),
        `accepted artifact ${target} must be unchanged`,
      ).toBe(BATCH_3_SITUATION_PINS[target]!.accepted);
    }
    expect(
      digestArtifact(readFileSync(join(BATCH_3_DIR, "index.json"), "utf-8")),
    ).toBe(BATCH_3_INDEX_PIN.accepted);
  });
  ...
  it("a fresh run to temp dir reproduces the pinned post-fix artifacts and verdicts", () => {
    ...
    for (const target of BATCH_3_TARGETS) {
      const batch3Content = readFileSync(
        join(BATCH_3_DIR, `${target}.json`),
        "utf-8",
      );
      ...
      ).toBe(BATCH_3_SITUATION_PINS[target]!.live);

      // The accepted honest verdict is reproduced by the live run, so the
      // binding still proves the claim rather than only its bytes.
      const acceptedArtifact = JSON.parse(batch3Content) as SituationEvidenceArtifact;
  ...
  it("re-run produces the pinned index.json and matches the accepted verdicts", () => {
    const batch3Index = readFileSync(
      join(BATCH_3_DIR, "index.json"),
      "utf-8",
    );
    ...
    expect(digestArtifact(batch3Index)).toBe(BATCH_3_INDEX_PIN.accepted);
    expect(digestArtifact(freshIndex)).toBe(BATCH_3_INDEX_PIN.live);

    const acceptedEntries = JSON.parse(batch3Index).situations as Array<{
      situation_id: string;
      verdict: string;
  ...
  it("re-run produces the pinned trajectory hash chain over the same tick count", () => {
    ...
    for (const target of BATCH_3_TARGETS) {
      const batch3Artifact = readFileSync(
        join(BATCH_3_DIR, `${target}.json`),
        "utf-8",
      );
      const freshArtifact = readFileSync(
        join(tmpDir, `${target}.json`),
        "utf-8",
      );

      const acceptedChain = (JSON.parse(batch3Artifact).trajectory as Array<{ hash: string }>).map(
        (entry) => entry.hash,
      );
      const freshChain = (JSON.parse(freshArtifact).trajectory as Array<{ hash: string }>).map(
        (entry) => entry.hash,
      );

      expect(digestTrajectoryChain(batch3Artifact)).toBe(
        BATCH_3_TRAJECTORY_PINS[target]!.accepted,
      );
      expect(digestTrajectoryChain(freshArtifact)).toBe(
        BATCH_3_TRAJECTORY_PINS[target]!.live,
      );
      // Same window, same number of committed ticks — only the states moved.
  ...
  it("two fresh runs are byte-identical to each other", () => {
    runSituationEvaluator("3v3-situation-driven-extended.v1.json", tmpDir);
    const first = BATCH_3_TARGETS.map((target) =>
      digestArtifact(readFileSync(join(tmpDir, `${target}.json`), "utf-8")),
    );

    runSituationEvaluator("3v3-situation-driven-extended.v1.json", tmpDir);
    const second = BATCH_3_TARGETS.map((target) =>
      digestArtifact(readFileSync(join(tmpDir, `${target}.json`), "utf-8")),
    );
...
describe("BATCH-3 binding: verdict computation", () => {
...
describe("BATCH-3 binding: index metadata", () => {
...
// ---------------------------------------------------------------------------
// 6. Extended fixture produces second-touch events (all_events)
// ---------------------------------------------------------------------------
...
describe("BATCH-3 binding: artifact shape", () => {
