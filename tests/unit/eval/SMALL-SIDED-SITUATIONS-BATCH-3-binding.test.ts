...
describe("BATCH-3 binding: byte-identical re-run", () => {
  it("a fresh run to temp dir produces byte-identical artifacts", () => {
    ...
    for (const target of BATCH_3_TARGETS) {
      ...
      expect(freshContent).toBe(
        batch3Content,
        `Artifact ${target} from fresh run must match batch-3 persisted artifact`,
      );
  ...
  it("re-run produces identical index.json", () => {
    ...
    expect(freshIndex).toBe(batch3Index);
  });
  ...
  it("re-run produces identical trajectory hashes", () => {
    ...
    for (const target of BATCH_3_TARGETS) {
      const batch3Artifact = JSON.parse(
        readFileSync(join(BATCH_3_DIR, `${target}.json`), "utf-8"),
      ) as SituationEvidenceArtifact;
      ...
      expect(freshArtifact.trajectory.map((t) => t.hash)).toEqual(
        batch3Artifact.trajectory.map((t) => t.hash),
      );
    }
  });
});
