...
- trajectory_regeneration:
  - **timestamp**: 2026-08-25T23:54Z (consequence of SMALL-SIDED-CONTINUOUS-DUEL-AND-SHOT-CLOSURE ball system fix)
  - **cause**: `src/simulation/ball/ball-system.ts` added ground-roll/settled → airborne transition when `linearVelocity.z > 0`. The 3v3 press match includes shots; the ball regime transition changes post-shot dynamics, producing different trajectory tick data.
  - **method**: Re-generated by running the PRESS-AND-SUPPORT-DEPTH binding tests (test #2 runs the match via `runHeadlessMatch` and writes trajectory.json). The trajectory test asserts byte-identical re-run consistency; 14/14 PRESS binding tests pass against the regenerated file.
  - **scope**: trajectory.json only (18,006 lines changed). No other PRESS artifacts affected.
  - **regression**: 14/14 PRESS binding tests PASS, 31/31 scanner tests PASS
- known_gaps:
