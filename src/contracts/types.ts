...
export type ActionKind = 'pass' | 'through' | 'cross' | 'shoot' | 'tackle' | 'header';
export type Phase = 'kickoff' | 'playing' | 'goal' | 'throw-in' | 'corner' | 'goal-kick' | 'free-kick' | 'offside' | 'halftime' | 'fulltime';
...
export interface Capabilities {
  ...
  curve: number; weakFoot: number; keeper: number;
}
...
export interface Stats { shots: number; onTarget: number; passes: number; completed: number; saves: number; tackles: number; fouls: number; possessionTicks: number; corners: number; offsides: number }
export interface Config {
  ...
  rules: { restartTicks: number; celebrationTicks: number; halftimeTicks: number; offside: boolean; fouls: boolean };
  goalkeeper: { radius: number; lateral: number; speed: number; reach: number; reactionTicks: number; releaseTicks: number };
}
...
export interface Replay { version: 'replay-v1'; initial: World; inputs: InputFrame[]; endTick: number; finalHash: string }
