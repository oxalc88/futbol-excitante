...
export class Recorder {
  ...
  finish(sim:Simulation):Replay{return {version:'replay-v1',initial:this.initial,inputs:this.inputs,endTick:sim.tick,finalHash:sim.stateHash()};}
}
export function parseReplay(value:unknown):Replay {
  const r=value as Replay;
  if(!r||r.version!=='replay-v1'||!r.initial||!Array.isArray(r.inputs)||r.inputs.length>250000||!Number.isInteger(r.endTick)||r.endTick<r.initial.tick||r.endTick>250000||!/^\w{8}$/.test(r.finalHash))throw new Error('Repetición inválida o demasiado larga.');
