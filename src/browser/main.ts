...
<main class="game">
 ...
 <section id="viewport" aria-label="Partido de fútbol">
  ...
  <div class="touch-controls"><div id="joystick" aria-label="Joystick táctil"><div id="stick"></div></div><div class="touch-buttons"><button data-action="cross">Centro</button><button data-action="lob">Elevado</button><button data-action="header">Cabeza</button><button data-action="slide">Barrida</button><button id="touch-shield">Proteger</button><button data-action="through">Filtrado</button><button id="touch-sprint">Correr</button><button data-action="tackle">Entrada</button><button data-action="pass">Pase</button><button id="touch-shoot" class="shoot">Tiro</button><button id="touch-switch">Cambiar</button></div></div>
 </section>
 <footer><span id="footer-match">ATLÉTICO COSTA <i>vs</i> DEPORTIVO SIERRA</span><span id="footer-size">11 contra 11 · Equipos ficticios</span><button id="help-btn">Controles</button></footer>
</main>
...
let renderError='';let audioContext:AudioContext|null=null,sound=false;let notice='',noticeUntil=0;
const tacticalKeys=['pressing','width','tempo','counterPress','shortPossession','tightMarking','adaptation','defensiveLine','compactness','support']as const;
const phaseLabels:Record<string,string>={kickoff:'Saque inicial',playing:'En juego',goal:'¡GOL!',halftime:'Entretiempo',fulltime:'Final del partido','throw-in':'Saque de banda',corner:'Tiro de esquina','goal-kick':'Saque de meta','free-kick':'Tiro libre',offside:'Fuera de juego',penalty:'Penalti · apunta y tira',indirect:'Tiro libre indirecto'};
...
function makeRenderer(k:[Kit,Kit]){renderer?.dispose();renderError='';try{renderer=new FootballRenderer($('viewport'),$<HTMLCanvasElement>('radar'),config,k);renderer.setQuality($<HTMLSelectElement>('quality').value);}catch(e){renderer=null;renderError='No se pudo iniciar WebGL2. Prueba un navegador compatible con aceleración gráfica.';$('announcement').textContent=renderError;console.error(e);}}
...
function closeDialog(id:string){$<HTMLDialogElement>(id).close();if(started&&!guest&&!document.querySelector('dialog[open]'))pause(current.phase==='fulltime'||!!replayResult);}
...
function start(mode?:string){
  ...
  for(const k of tacticalKeys)t1.tactics[k]=Number($<HTMLInputElement>(k).value);
...
document.querySelectorAll<HTMLDialogElement>('dialog').forEach(d=>d.addEventListener('cancel',()=>{if(started)queueMicrotask(()=>pause(current.phase==='fulltime'||!!replayResult));}));
...
window.addEventListener('keydown',e=>{if((e.target as HTMLElement)?.matches('input,textarea,select')||document.querySelector('dialog[open]'))return;if(e.repeat)return;if(e.code==='KeyP'||e.code==='Escape')pause(!paused);if(e.code==='KeyC')changeCamera();});
...
function updateHud(){
  ...
  $('announcement').textContent=renderError||replayResult||(s.phase==='goal'?'¡GOOOL!':s.phase==='halftime'?'ENTRETIEMPO':s.phase==='fulltime'?'FINAL DEL PARTIDO':s.tick<noticeUntil?notice:'');
  $('footer-size').textContent=`${config.teamSize} contra ${config.teamSize} · Equipos ficticios`;
...
$<HTMLInputElement>('replay-file').onchange=async e=>{try{const f=(e.target as HTMLInputElement).files?.[0];if(!f)return;if(f.size>30000000)throw new Error('La repetición supera 30 MB.');const loaded=parseReplay(JSON.parse(await f.text()));loaded.initial.teams.forEach(validateTeam);peer.close();sim=replaySimulation(loaded);config=loaded.initial.config;record=new Recorder(sim);replay=loaded;replayIndex=0;replayResult='';notice='';started=true;guest=false;finalShown=false;previous=current=sim.presentation();makeRenderer(kits(0,1));$('setup-dialog').closest('dialog')?.close();pause(false);$('match-mode').textContent='REPETICIÓN';}catch(e){$('setup-error').textContent=(e as Error).message;}};
for(const key of ['formation',...tacticalKeys])$(key).addEventListener('change',()=>{if(!started||guest||replay)return;pendingTactics={formation:$<HTMLSelectElement>('formation').value as TeamProfile['tactics']['formation'],pressing:Number($<HTMLInputElement>('pressing').value),...Object.fromEntries(tacticalKeys.map(k=>[k,Number($<HTMLInputElement>(k).value)]))};});
...
joystick.onlostpointercapture=joystick.onpointerup=joystick.onpointercancel=()=>{stickPointer=null;input.setTouchMove({x:0,y:0});stick.style.transform='';};
...
$('touch-sprint').onpointerdown=e=>{(e.target as HTMLElement).setPointerCapture(e.pointerId);input.setTouchSprint(true);};$('touch-sprint').onlostpointercapture=$('touch-sprint').onpointerup=$('touch-sprint').onpointercancel=()=>input.setTouchSprint(false);$('touch-switch').onclick=()=>input.switchTouch();
$('touch-shoot').onpointerdown=e=>{e.preventDefault();$('touch-shoot').setPointerCapture(e.pointerId);input.startTouchShot();};
$('touch-shoot').onpointerup=()=>input.endTouchShot();$('touch-shoot').onpointercancel=$('touch-shoot').onlostpointercapture=()=>input.endTouchShot(true);
$('touch-shield').onpointerdown=e=>{e.preventDefault();$('touch-shield').setPointerCapture(e.pointerId);input.setTouchShield(true);};$('touch-shield').onpointerup=$('touch-shield').onpointercancel=$('touch-shield').onlostpointercapture=()=>input.setTouchShield(false);
$('touch-option').onchange=()=>{document.body.dataset.touch=$<HTMLSelectElement>('touch-option').value;};
$('quality').onchange=()=>renderer?.setQuality($<HTMLSelectElement>('quality').value);
...
function loop(now:number){
  ...
  if(started&&!document.querySelector('dialog[open]')&&input.pauseRequested())pause(!paused);
  try{
    if(guest){if(peer.connected&&!paused){guestCounter++;const frame=input.sample(guestCounter,0);peer.input({...frame,slot:1});}renderer?.draw(previous,current,Math.min(1,(now-networkTick)/100),elapsed);}
      ...
      if(!paused){accumulator+=elapsed;let steps=0;while(accumulator>=config.dt&&steps<6){
        ...
        sim.applyInputs(frames);if(!replay)record.record(frames);previous=current;sim.step();current=sim.presentation();renderer?.advanceCamera(current);
        const alert=current.events.find(e=>['yellow-card','red-card','advantage','advantage-recalled','keeper-time-limit'].includes(e.type));if(alert){const labels:Record<string,string>={'yellow-card':'AMARILLA','red-card':'EXPULSIÓN',advantage:'LEY DE VENTAJA','advantage-recalled':'SE COBRA LA FALTA','keeper-time-limit':'OCHO SEGUNDOS · CÓRNER'};notice=labels[alert.type];noticeUntil=current.tick+100;}
